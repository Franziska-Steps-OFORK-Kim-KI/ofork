...
Core.UI.Popup = (function (TargetNS) {
     ...
     * @description
     *      Checks if current window is an OFORK popup.
     */
    function CurrentIsPopupWindow() {
        var PopupType;

        if (window.name.match(/OFORKPopup_([^_]+)_.+/)) {
            PopupType = RegExp.$1;
        }
    ...
    TargetNS.RegisterPopupAtParentWindow = function (WindowObject) {
        ...
        /OFORKPopup_([^_]+)_.*/.exec(WindowObject.name);
        Type = RegExp.$1;
        ...
        if (URL) {
            ...
            if (ConfirmClosePopup) {
                ...
                if (Unlinked && Unlinked === 1) {
                    WindowName = 'PopupOFORK_' + Type + '_' + Date.parse(new Date());
                }
                else {
                    WindowName = 'OFORKPopup_' + Type + '_' + Date.parse(new Date());
                }
    ...
    TargetNS.ClosePopup = function (PopupType) {
        ...
        if (typeof PopupType === 'undefined') {
            ...
            // in some circumstances the parameter PopupType is an window object instead of a string
                ...
                if (PopupObject && typeof PopupObject.name !== 'undefined' && PopupObject.name.match(/OFORKPopup_([^_]+)_.+/)) {
                    PopupType = RegExp.$1;
                }
