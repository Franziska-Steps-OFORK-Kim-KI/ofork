...
Core.Agent.CustomerSearch = (function (TargetNS) {
    ...
    function GetCustomerInfo(CustomerUserID) {
        ...
        Core.AJAX.FunctionCall(Core.Config.Get('Baselink'), Data, function (Response) {
            ...
            // only execute this part, if in AgentTicketEmail or AgentTicketPhone or AgentTicketRequest or AgentProcessTicket
            if (Core.Config.Get('Action') === 'AgentTicketEmail' || Core.Config.Get('Action') === 'AgentTicketPhone' || Core.Config.Get('Action') === 'AgentTicketRequest' || Core.Config.Get('Action') === 'AgentProcessTicket') {
                // reset service
                $('#ServiceID').attr('selectedIndex', 0);

                var IFConfigItem = $("#IfConfigItem").val();
                if (IFConfigItem) {
                    GetCustomerCMDB(CustomerUserID);
                }
    ...
    function GetCustomerCMDB(CustomerUserID) {

		    var SessionName = Core.Config.Get('SessionName');
		    var SessionID = Core.Config.Get('SessionID');
            ...
            $.ajax({
                ...
                data:
                {
                    Action:            'AgentCMDBTicketItemSearch',
                    Subaction:         'TicketItemSearch',
                    CustomerUserID: CustomerUserID
                },
                async: false,
                dataType: 'html',
                success: function(Response) {
                    $('#CustomerCMDBItems').html(Response);
                }
            });
    }
    ...
    function GetCustomerTickets(CustomerUserID, CustomerID) {
        ...
        function ReplaceCustomerTicketLinks() {
            ...
            // Events for Customer History table - AgentTicketPhone, AgentTicketEmail, AgentTicketRequest, AgentProcessTicket and AgentTicketCustomer screens.
            if (
                Core.Config.Get('Action') === 'AgentTicketPhone' ||
                Core.Config.Get('Action') === 'AgentTicketRequest' ||
                Core.Config.Get('Action') === 'AgentProcessTicket' ||
    ...
    function CheckPhoneCustomerCountLimit() {
        ...
        if (Core.Config.Get('Action') !== 'AgentTicketPhone' || Core.Config.Get('Action') !== 'AgentTicketRequest') {
            return;
        }
    ...
    TargetNS.Init = function ($Element) {
        ...
        // get customer tickets for AgentTicketPhone, AgentTicketRequest, AgentProcessTicket and AgentTicketEmail
        if ((Core.Config.Get('Action') === 'AgentTicketEmail' || Core.Config.Get('Action') === 'AgentTicketPhone' || Core.Config.Get('Action') === 'AgentTicketRequest' || Core.Config.Get('Action') === 'AgentProcessTicket') && $('#SelectedCustomerUser').val() !== '') {
        ...
        if (isJQueryObject($Element)) {
            ...
            $Element.off('keyup.Validate').on('keyup.Validate', function () {
               ...
               if ($Element.hasClass('ServerError') && Value.length) {
                   $('#OFORK_UI_Tooltips_ErrorTooltip').hide();
               }
            });
