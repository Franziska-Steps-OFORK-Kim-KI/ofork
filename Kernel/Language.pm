...
# --
# $Id: Language.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub new {
    ...
    # take time zone
    $Self->{TimeZone} = $Param{UserTimeZone} || $Param{TimeZone} || Kernel::System::DateTime->OFORKTimeZoneGet();
...
sub FormatTimeString {
    ...
    if ( $String =~ /(\d{4})-(\d{2})-(\d{2})\s(\d{2}):(\d{2}):(\d{2})/ ) {
        ...
        # output time zone only if it differs from OFORK' time zone
        if (
            $Config ne 'DateFormatShort'
            && $Self->{TimeZone}
            && $Self->{TimeZone} ne Kernel::System::DateTime->OFORKTimeZoneGet()
            )
        {
