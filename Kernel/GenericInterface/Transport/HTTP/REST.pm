...
# --
# $Id: REST.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub ProviderProcessRequest {
    ...
    my %DataHeaderOverwrite = (
        HTTP_X_OFORK_HEADER_USERLOGIN         => 'UserLogin',
        HTTP_X_OFORK_HEADER_CUSTOMERUSERLOGIN => 'CustomerUserLogin',
        HTTP_X_OFORK_HEADER_SESSIONID         => 'SessionID',
        HTTP_X_OFORK_HEADER_PASSWORD          => 'Password',
        HTTP_X_OFORK_HEADER_IMPERSONATEASUSER => 'ImpersonateAsUser',
    );
...
sub RequesterPerformRequest {
        ...
        )
    {
            ...
            )
        {
            ...
            for my $X509CertificateKey ( sort keys %{$X509Certificate} ) {
                ...
                $JWTPlaceholderData{ 'OFORK_JWT_Cert' . $X509CertificateKey } = $X509CertificateValue;
            }
        }
        ...
        $JWTPlaceholderData{OFORK_JWT_ExpirationDateTimestamp} = $DateTimeObject->ToEpoch();
        $JWTPlaceholderData{OFORK_JWT_ExpirationDateString}    = $DateTimeObject->Format(
    ...
    if ( IsHashRefWithData( $Param{Data} ) ) {
            ...
            )
        {
            ...
            if ( $Headers->{'Content-Type'} eq 'application/x-www-form-urlencoded' ) {
                ...
                # Default to JSON (OFORK default behavior)
    ...
    if ( IsHashRefWithData( $Config->{AdditionalHeaders} ) ) {
        ...
        # Currently, the only placeholder supported is OFORK_JWT.
        my %PlaceholderData;
        if (
            $JWTObjectIsSupported
            && IsHashRefWithData( $Config->{Authentication} )
