...
# --
# $Id: ObjectManager.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
The ObjectManager is the central place to create and access singleton OFORK objects (via C<L</Get()>>)
...
The ObjectManager must always be provided to OFORK by the top level script like this:

    use Kernel::System::ObjectManager;
    local $Kernel::OM = Kernel::System::ObjectManager->new(
        # possible options for module constructors here
        LogObject {
            LogPrefix => 'OFORK-MyTestScript',
        },
    );
...
This is typically B<only> needed in top level (C<bin/>) scripts! All parts of the OFORK API assume
...
sub ObjectsDiscard {
    ...
    # During an OFORK package upgrade the packagesetup code module has just
