# --
# Kernel/System/TemplateGenerator.pm
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# --
# $Id: TemplateGenerator.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Salutation {
    ...
    my @ListOfUnSupportedTag = qw(OFORK_AGENT_SUBJECT OFORK_AGENT_BODY OFORK_CUSTOMER_BODY OFORK_CUSTOMER_SUBJECT);
...
sub Signature {
    ...
    my %SignatureMemberList = $Kernel::OM->Get('Kernel::System::Signature')->UserSignatureMemberList(
        UserLogin => $Param{UserID},
        Result    => 'HASH',
    );

    my $SetNewSignatureID = '';
    for my $SetSignatureID ( keys %SignatureMemberList ) {
        $SetNewSignatureID = $SetSignatureID;
    }

    if ( $SetNewSignatureID ) {

        %Signature = ();

        # Get signature.
        %Signature = $Kernel::OM->Get('Kernel::System::Signature')->SignatureGet(
            ID => $SetNewSignatureID,
        );
    }
...
replace all OFORK smart tags in the notification body and subject
...
sub NotificationEvent {
    ...
    # replace <OFORK_CUSTOMER_DATA_*> tags early from CustomerMessageParams, the rests will be replaced
    ...
    for my $Key ( sort keys %{ $Param{CustomerMessageParams} || {} } ) {
        ...
        $Notification{Body}    =~ s/${Start}OFORK_CUSTOMER_DATA_$Key${End}/$Param{CustomerMessageParams}->{$Key}/gi;
        $Notification{Subject} =~ s/<OFORK_CUSTOMER_DATA_$Key>/$Param{CustomerMessageParams}->{$Key}{$Key}/gi;
    }
...
sub _Replace {
    ...
    if ( $Param{AddTimezoneInfo} ) {
        $RecipientTimeZone = $Kernel::OM->Create('Kernel::System::DateTime')->OFORKTimeZoneGet();
    ...
    my $HashGlobalReplace = sub {
        ...
        #   e. g. <OFORK_CUSTOMER_From> and <OFORK_CUSTOMER_FROM>.
        %H = map {
        ...
        # If tag is 'OFORK_CUSTOMER_' add the body alias 'email/note' to be replaced.
        if ( $Tag =~ m/OFORK_(CUSTOMER|AGENT)_/ ) {
            KEY:
    ...
    # get recipient data and replace it with <OFORK_...
    $Tag = $Start . 'OFORK_';

    # include more readable tag <OFORK_NOTIFICATION_RECIPIENT
    my $RecipientTag = $Start . 'OFORK_NOTIFICATION_RECIPIENT_';
    ...
    # get owner data and replace it with <OFORK_OWNER_...
    $Tag = $Start . 'OFORK_OWNER_';

    # include more readable version <OFORK_TICKET_OWNER
    my $OwnerTag = $Start . 'OFORKTICKET_OWNER_';
    ...
    # get owner data and replace it with <OFORK_RESPONSIBLE_...
    $Tag = $Start . 'OFORK_RESPONSIBLE_';

    # include more readable version <OFORK_TICKET_RESPONSIBLE
    my $ResponsibleTag = $Start . 'OFORK_TICKET_RESPONSIBLE_';
    ...
    $Tag = $Start . 'OFORK_Agent_';
    my $Tag2        = $Start . 'OFORK_CURRENT_';
    ...
    $Param{Text} =~ s/$Start OFORK_FIRST_NAME $End/$CurrentUser{UserFirstname}/gxms;
    $Param{Text} =~ s/$Start OFORK_LAST_NAME $End/$CurrentUser{UserLastname}/gxms;

    # cleanup
    $Param{Text} =~ s/$Tag2.+?$End/-/gi;

    # ticket data
    $Tag = $Start . 'OFORK_TICKET_';
    ...
    # <OFORK_TICKET_DynamicField_NameX> returns the stored key
    # <OFORK_TICKET_DynamicField_NameX_Value> returns the display value
    ...
    # COMPAT
    $Param{Text} =~ s/$Start OFORK_TICKET_ID $End/$Ticket{TicketID}/gixms;
    $Param{Text} =~ s/$Start OFORK_TICKET_NUMBER $End/$Ticket{TicketNumber}/gixms;
    if ( $Ticket{TicketID} ) {
        $Param{Text} =~ s/$Start OFORK_QUEUE $End/$Ticket{Queue}/gixms;
    }
    if ( $Param{QueueID} ) {
        $Param{Text} =~ s/$Start OFORK_TICKET_QUEUE $End/$Queue{Name}/gixms;
    }

    # cleanup
    $Param{Text} =~ s/$Tag.+?$End/-/gi;

    # get customer and agent params and replace it with <OFORK_CUSTOMER_... or <OFORK_AGENT_...
    my %ArticleData = (
        'OFORK_CUSTOMER_' => $Param{Data}      || {},
        'OFORK_AGENT_'    => $Param{DataAgent} || {},
    );

    # use a list to get customer first
    for my $DataType (qw(OFORK_CUSTOMER_ OFORK_AGENT_)) {
        ...
        if (%Data) {
            ...
            # replace <OFORK_CUSTOMER_*> and <OFORK_AGENT_*> tags
            $Tag = $Start . $DataType;
            $HashGlobalReplace->( $Tag, %Data );

            # prepare body (insert old email) <OFORK_CUSTOMER_EMAIL[n]>, <OFORK_CUSTOMER_NOTE[n]>
            #   <OFORK_CUSTOMER_BODY[n]>, <OFORK_AGENT_EMAIL[n]>..., <OFORK_COMMENT>
            ...
            while (
                ...
                ||
                $Param{Text} =~ /$Start(?:OFORK_COMMENT(\[(.+?)\])?)$End/
                )
            {
                ...
                # replace tag
                $Param{Text}
                    =~ s/$Start(?:(?:$DataType(EMAIL|NOTE|BODY)\[(.+?)\]|(?:OFORK_COMMENT(\[(.+?)\])?)))$End/$NewOldBody/;
            }

            # replace <OFORK_CUSTOMER_SUBJECT[]>  and  <OFORK_AGENT_SUBJECT[]> tags
            ...
            if ( $DataType eq 'OFORK_CUSTOMER_' ) {

                # get <OFORK_EMAIL_DATE[]> from body and replace with received date
                use POSIX qw(strftime);
                $Tag = $Start . 'OFORK_EMAIL_DATE';
        ...
        if ( $DataType eq 'OFORK_CUSTOMER_' ) {
            ...
            $Tag = $Start . 'OFORK_CUSTOMER_REALNAME';
            if ( $Param{Text} =~ /$Tag$End/i ) {
                ...
                # replace <OFORK_CUSTOMER_REALNAME> with from
                $Param{Text} =~ s/$Tag$End/$From/g;
            }
        }
    }

    # get customer data and replace it with <OFORK_CUSTOMER_DATA_...
    $Tag  = $Start . 'OFORK_CUSTOMER_';
    $Tag2 = $Start . 'OFORK_CUSTOMER_DATA_';
    ...
    # cleanup all not needed <OFORK_CUSTOMER_DATA_ tags
    $Param{Text} =~ s/(?:$Tag|$Tag2).+?$End/-/gi;

    # cleanup all not needed <OFORK_AGENT_ tags
    $Tag = $Start . 'OFORK_AGENT_';
    $Param{Text} =~ s/$Tag.+?$End/-/gi;
