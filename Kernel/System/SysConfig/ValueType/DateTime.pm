# --
# Kernel/System/SysConfig/ValueType/DateTime.pm
...
sub SettingEffectiveValueCheck {
    ...
    my $OFORKTimeZone = $Kernel::OM->Get('Kernel::Config')->Get("OFORKTimeZone");
    my $DateTimeObject;

    if ( !$Preferences{UserTimeZone} || $Preferences{UserTimeZone} eq $OFORKTimeZone ) {
        $DateTimeObject = $Kernel::OM->Create(
            'Kernel::System::DateTime',
            ObjectParams => {
                TimeZone => $OFORKTimeZone,
            },
        );
        ...
        if ($Success) {
            ...
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "System was not able to calculate user DateTime in OFORKTimeZone!"
            );
        }
    }
...
sub SettingRender {
    ...
    my $TimeZone = $Kernel::OM->Get('Kernel::Config')->Get("OFORKTimeZone");
