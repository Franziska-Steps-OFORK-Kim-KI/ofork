...
# --
# $Id: SysConfig.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
Returns:
    ...
    %Setting = (
        ...
        OverriddenFileName        => '/opt/ofork/Kernel/Config/Files/ZZZ.pm',
    );
...
sub SettingUpdate {
    ...
    if (
        !$Param{IsValid}
        && !$Param{TargetUserID}
        && $Self->can('UserSettingValueDelete')
        )
    {
...
sub ConfigurationXML2DB {
    ...
    for my $File (@Files) {
        ...
        # Check ofork_config Init attribute.
        $$ConfigFile =~ m{<ofork_config.*?init="(.*?)"}gsmx;
        my $InitValue = $1;
        ...
        if ( !defined $SettingsByInit{$InitValue} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message =>
                    "Invalid ofork_config Init value ($InitValue)! Allowed values: Framework, Application, Config, Changes.",
            );
            next FILE;
        }
...
sub ConfigurationDeploy {
    ...
    if ($AddNewDeployment) {
        ...
        if ( $Self->can('UserConfigurationResetToGlobal') ) {
...
sub ConfigurationDeploySync {
    ...
    return 1 if !$Self->can('UserConfigurationDeploySync');
...
sub ConfigurationDump {
    ...
    if ( !$Param{SkipModifiedSettings} || !$Param{SkipUserSettings} ) {
        ...
        if ( !$Param{SkipUserSettings} && $Self->can('UserConfigurationDump') ) {
...
sub ConfigurationLoad {
    ...
    for my $Section ( sort keys %Configuration ) {
        ...
        next SECTION if !$Self->can('UserConfigurationDeploy');
...
Returns:
    ...
    %Categories = (
        ...
        OFORK => {
            DisplayName => 'OFORK',
            Files       => ['Calendar.xml', 'Daemon.xml', 'Framework.xml', 'GenericInterface.xml', 'ProcessManagement.xml', 'Ticket.xml' ],
        },
        # ...
    );
...
sub OverriddenFileNameGet {
    ...
    $EffectiveValue =~ s/\<OFORK_CONFIG_(.+?)\>/$ConfigObject->{$1}/g;
...
sub _EffectiveValues2PerlFile {
    ...
    my $FileStrg = <<"EOF";
# OFORK config file (automatically generated)
# VERSION:2.0
package $TargetPath;
use strict;
use warnings;
