...
# --
# $Id: PostMaster.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
Don't use the constructor directly, use the ObjectManager instead:
    ...
    my $PostMasterObject = $Kernel::OM->Create(
        ...
        ObjectParams => {
            ...
            Trusted      => 1, # 1|0 ignore X-OFORK header if false
        },
    );
...
sub new {
    ...
    if ( $Self->{Trusted} ) {
        ...
        for my $DynamicField ( values %$DynamicFields ) {
            for my $Header (
                'X-OFORK-DynamicField-' . $DynamicField,
                'X-OFORK-FollowUp-DynamicField-' . $DynamicField,
                )
            {
...
return params
    ...
    5 = ignored (because of X-OFORK-Ignore header)
...
sub Run {
    ...
    if ( $GetParam->{'X-OFORK-Ignore'} && $GetParam->{'X-OFORK-Ignore'} =~ /(yes|true)/i ) {
        $Self->{CommunicationLogObject}->ObjectLog(
            ObjectLogType => 'Message',
            Priority      => 'Info',
            Key           => 'Kernel::System::PostMaster',
            Value =>
                ...
                . "because the X-OFORK-Ignore is set (X-OFORK-Ignore: $GetParam->{'X-OFORK-Ignore'}).",
        );
        return (5);
    }
...
sub GetEmailParams {
    ...
    for my $Param ( @{ $Self->{'PostmasterX-Header'} } ) {
        ...
        # do not scan x-ofork headers if mailbox is not marked as trusted
        next HEADER if ( !$Self->{Trusted} && $Param =~ /^x-ofork/i );
    ...
    for my $Key (qw(X-OFORK-SenderType X-OFORK-FollowUp-SenderType)) {

        if ( !$GetParam{$Key} ) {
            $GetParam{$Key} = 'customer';
        }

        # check if X-OFORK-SenderType exists, if not, set customer
    ...
    for my $Key (qw(X-OFORK-IsVisibleForCustomer X-OFORK-FollowUp-IsVisibleForCustomer)) {
