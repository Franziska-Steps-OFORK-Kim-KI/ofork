# --
# Kernel/System/ProcessManagement/TransitionAction/Base.pm
...
sub _ReplaceTicketAttributes {
    ...
    for my $Attribute ( sort keys %{ $Param{Config} } ) {
        ...
        # <OFORK_Ticket_DynamicField_Name1> or <OFORK_TICKET_DynamicField_Name1>
        # or
        # <OFORK_TICKET_DynamicField_Name1_Value> or <OFORK_Ticket_DynamicField_Name1_Value>.
        # <OFORK_Ticket_*> is deprecated and should be removed in further versions of OFORK.
        my $Count = 0;
        REPLACEMENT:
        while (
            $Param{Config}->{$Attribute}
            && $Param{Config}->{$Attribute} =~ m{<OFORK_TICKET_([A-Za-z0-9_]+)>}msxi
            && $Count++ < 1000
            )
        {
            ...
            if ( $TicketAttribute =~ m{DynamicField_(\S+?)_Value} ) {
                ...
                $Param{Config}->{$Attribute}
                    =~ s{<OFORK_TICKET_$TicketAttribute>}{$DisplayValueStrg->{Value} // ''}ige;
                ...
                $Param{Config}->{$Attribute}
                    =~ s{<OFORK_TICKET_$TicketAttribute>}{$ValueStrg->{Value} // ''}ige;

                next REPLACEMENT;
            }

            # if ticket value is scalar substitute all instances (as strings)
            # this will allow replacements for "<OFORK_TICKET_Title> <OFORK_TICKET_Queue"
            if ( !ref $Param{Ticket}->{$TicketAttribute} ) {
                $Param{Config}->{$Attribute}
                    =~ s{<OFORK_TICKET_$TicketAttribute>}{$Param{Ticket}->{$TicketAttribute} // ''}ige;
            }
            else {
...
sub _ReplaceAdditionalAttributes {
    ...
    # start replacing of OFORK smart tags
    for my $Attribute ( sort keys %{ $Param{Config} } ) {

        my $ConfigValue = $Param{Config}->{$Attribute};

        if ( $ConfigValue && $ConfigValue =~ m{<OFORK_[A-Za-z0-9_]+(?:\[(?:.+?)\])?>}smxi ) {
