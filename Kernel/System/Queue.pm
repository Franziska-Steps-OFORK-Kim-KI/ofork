...
sub QueueAdd {
    ...
    if ( !$Param{NoDefaultValues} ) {
        for (
            ...
            FollowUpLock SystemAddressID SalutationID SignatureID SupervisorID
    ...
    return if !$DBObject->Do(
        SQL => 'INSERT INTO queue (name, group_id, unlock_timeout, system_address_id, '
            ...
            . ' update_notify, solution_time, solution_notify, sw_id, follow_up_id, '
        ...
        Bind => [
            ...
            \$Param{FirstResponseTime}, \$Param{FirstResponseNotify},
            \$Param{UpdateTime}, \$Param{UpdateNotify}, \$Param{SolutionTime}, \$Param{SolutionNotify},
            \$Param{SupervisorID},
...
sub QueueGet {
    ...
    my $SQL = 'SELECT q.name, q.group_id, q.unlock_timeout, '
        ...
        . 'q.default_sign_key, q.calendar_name, q.create_time, q.change_time, '
        . 'q.sw_id FROM queue q, system_address sa WHERE q.system_address_id = sa.id AND ';
...
sub QueueUpdate {
    ...
    if ( !$Param{SupervisorID} ) {
        $Param{SupervisorID} = 0;
    }
    ...
    return if !$DBObject->Do(
        SQL => '
            ...
            SET name = ?, comments = ?, group_id = ?, unlock_timeout = ?, first_response_time = ?,
                ...
                calendar_name = ?, default_sign_key = ?, salutation_id = ?, signature_id = ?,
                valid_id = ?, change_time = current_timestamp, change_by = ?, sw_id = ?
            WHERE id = ?',
        Bind => [
