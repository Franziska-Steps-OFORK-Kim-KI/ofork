...
# --
# $Id: InterfaceCustomer.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    if ( $Param{Action} eq 'PreLogin' ) {
        ...
        if ( !$User ) {
            $Kernel::OM->ObjectParamAdd(
                'Kernel::Output::HTML::Layout' => {
                    SetCookies => {
                        OFORKBrowserHasCookie => $ParamObject->SetCookie(
        ...
        if ( $ParamObject->GetCookie( Key => 'OFORKBrowserHasCookie' ) ) {
            $Kernel::OM->ObjectParamAdd(
                'Kernel::Output::HTML::Layout' => {
                    BrowserHasCookie => 1,
                },
        ...
        $Kernel::OM->Get('Kernel::System::Scheduler')->TaskAdd(
            ...
            Data                     => {
                Object   => 'Kernel::System::SupportDataCollector::PluginAsynchronous::OFORK::ConcurrentUsers',
        ...
        # of the OFORK user's time zone offset
        ...
        my $OFORKUserTimeZoneOffset = $DateTimeObject->Format( Format => '%{offset}' ) / 60;
        ...
        # the user's OFORKS time zone setting and the one reported by the user's browser.
        # If there is a difference it can be evaluated later to e. g. show a message
        # for the user to check his OFORK time zone setting.
        my $UserTimeZoneOffsetDifference = abs( $OFORKUserTimeZoneOffset - $BrowserTimeZoneOffset );
        $SessionObject->UpdateSessionID(
            SessionID => $NewSessionID,
    ...
    # create new customer account
        ...
        for ( sort keys %GetParams ) {
            $Body =~ s/<OFORK_$_>/$GetParams{$_}/gi;
        }
