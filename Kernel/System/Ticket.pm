        ...
        return if !$DBObject->Prepare(
            SQL => '
                SELECT st.id, st.queue_id, st.ticket_state_id, st.ticket_lock_id, st.ticket_priority_id,
                    ...
                    st.create_by, st.change_by, st.process_id
                FROM ticket st
                WHERE st.id = ?',
            Bind  => [ \$Param{TicketID} ],
            Limit => 1,
...
sub TicketEscalationPreferences {
    ...
    if ( $Kernel::OM->Get('Kernel::Config')->Get('Ticket::Service') && $Ticket{SLAID} ) {
        ...
        if ( $Ticket{EscalationSolutionTime} && !$Escalation{SolutionTime} ) {
            $Escalation{SolutionTime} = 3;
        }
    }
...
sub TicketCustomerSet {
    ...
    if ( defined $Param{No} ) {
        ...
        if ($Ok) {
            ...
            my $OkTimeTracking = $DBObject->Do(
                SQL => 'UPDATE time_tracking_article SET customer_id = ? WHERE ticket_id = ?',
                Bind => [ \$Param{No}, \$Param{TicketID} ]
            );
        }
    }
...
=head2 TicketEscalationSet()

to set the ticket Escalation 
...
sub TicketEscalationSet {
    my ( $Self, %Param ) = @_;

    # check needed stuff
    for my $Needed (qw(TicketID UserID SystemTime)) {
        if ( !$Param{$Needed} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Needed!"
    ...
    return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'UPDATE ticket SET '
            . ' escalation_solution_time = ?, escalation_time = ?, change_by = ? WHERE id = ?',
        Bind => [ \$Param{SystemTime}, \$Param{SystemTime}, \$Param{UserID}, \$Param{TicketID} ],
    );
...
=head2 TicketProcessSet()

to set the ticket process
...
sub TicketProcessSet {
    my ( $Self, %Param ) = @_;


    # db update
    return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => 'UPDATE ticket SET process_id = ? WHERE id = ?',
        Bind => [ \$Param{ProcessID}, \$Param{TicketID} ],
    );
...
sub TicketMerge {
    ...
    $Body =~ s{<OFORK_TICKET>}{$MergeTicket{TicketNumber}}xms;
    $Body =~ s{<OFORK_MERGE_TO_TICKET>}{$MainTicket{TicketNumber}}xms;
...
=head2 TicketFlagSetCustomer()

set ticket flags

    my $Success = $TicketObject->TicketFlagSetCustomer(
        TicketID => 123,
        Key      => 'Seen',
        Value    => 1,
        UserID   => 123, # apply to this user
...
sub TicketFlagSetCustomer {
    ...
    # get flags
    my %Flag = $Self->TicketFlagGetCustomer(
        TicketID => $Param{TicketID},
        UserID   => $Param{UserID},
    );
    ...
    return if !$DBObject->Do(
        SQL => '
            DELETE FROM ticket_flag_customer
            WHERE ticket_id = ?
                AND ticket_key = ?
                AND user_id = ?',
        Bind => [ \$Param{TicketID}, \$Param{Key}, \$Param{UserID} ],
    ...
    return if !$DBObject->Do(
        SQL => '
            INSERT INTO ticket_flag_customer
            (ticket_id, ticket_key, ticket_value, create_time, user_id)
            VALUES (?, ?, ?, current_timestamp, ?)',
        Bind => [ \$Param{TicketID}, \$Param{Key}, \$Param{Value}, \$Param{UserID} ],
    );
    ...
    $Kernel::OM->Get('Kernel::System::Cache')->Delete(
        Type => $Self->{CacheType},
        Key  => 'TicketFlagCustomer::' . $Param{TicketID},
    );

    # event
    $Self->EventHandler(
        Event => 'TicketFlagSetCustomer',
        Data  => {
            TicketID => $Param{TicketID},
            Key      => $Param{Key},
            Value    => $Param{Value},
...
=head2 TicketFlagDeleteCustomer()

delete ticket flag

    my $Success = $TicketObject->TicketFlagDeleteCustomer(
        TicketID => 123,
        Key      => 'Seen',
        UserID   => 123,
    );
...
Events:
    TicketFlagDeleteCustomer

=cut

sub TicketFlagDeleteCustomer {
    my ( $Self, %Param ) = @_;
    ...
    if ( $Param{AllUsers} ) {
        ...
        # get all affected users
        my @AllTicketFlags = $Self->TicketFlagGetCustomer(
            TicketID => $Param{TicketID},
            AllUsers => 1,
        );
        ...
        for my $Record (@AllTicketFlags) {
            ...
            $Self->EventHandler(
                Event => 'TicketFlagDeleteCustomer',
                Data  => {
                    TicketID => $Param{TicketID},
                    Key      => $Param{Key},
                    UserID   => $Record->{UserID},
...
=head2 TicketFlagGetCustomer()

get ticket flags

    my %Flags = $TicketObject->TicketFlagGetCustomer(
        TicketID => 123,
        UserID   => 123,  # to get flags of one user
    );
...
sub TicketFlagGetCustomer {
    ...
    if ( !$Flags || ref $Flags ne 'HASH' ) {
        ...
        return if !$DBObject->Prepare(
            SQL => '
                SELECT user_id, ticket_key, ticket_value
                FROM ticket_flag_customer
                WHERE ticket_id = ?',
            Bind => [ \$Param{TicketID} ],
        );
