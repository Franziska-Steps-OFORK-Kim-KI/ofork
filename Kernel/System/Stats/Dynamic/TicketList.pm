...
# --
# $Id: TicketList.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub GetStatTable {
    ...
    for my $TicketID (@TicketIDs) {
        ...
        for my $Attribute ( @{$SortedAttributesRef} ) {
            ...
            if (
                $Param{TimeZone}
                && $Param{TimeZone} ne Kernel::System::DateTime->OFORKTimeZoneGet()
                && $Ticket{$Attribute}
                && $Ticket{$Attribute} =~ /\A(\d{4})-(\d{2})-(\d{2})\s(\d{2}):(\d{2}):(\d{2})\z/
                )
            {

                $Ticket{$Attribute} = $StatsObject->_FromOFORKTimeZone(
