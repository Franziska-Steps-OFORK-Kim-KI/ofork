...
# --
# $Id: Package.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub PackageOnlineList {
    ...
    my $Filelist;
        ...
        my $XML = $Self->_Download( URL => $Param{URL} . '/ofork.xml' );
        return if !$XML;
...
sub PackageOnlineGet {
    ...
    return $Self->_Download( URL => $Param{Source} . 'File=' . $Param{File} );
}
...
sub PackageVerify {
    ...
    return if !$WebUserAgentObject;

    my $JSONObject = $Kernel::OM->Get('Kernel::System::JSON');
    $Self->{PackageVerifyURL}= 'https://opms.o-fork.de/opms/public.pl?Action=PackageVerification;';

    # verify package at web server
    my %VerifyResponse = $WebUserAgentObject->Request(
        URL => $Self->{PackageVerifyURL} . 'Package=' . $Name . '::' . $Sum,
    );

    # decode JSON data
    my $VerifyResponseData = $JSONObject->Decode(
        Data => ${ $VerifyResponse{Content} },
    );
    my $PackageVerify = $VerifyResponseData->{$Name};
...
sub PackageVerifyAll {
    ...
    for my $Package (@PackageList) {
        ...
        if ($Verification) {
            ...
            push @PackagesToVerify, $Package->{Name};
        }
    }
    ...
    PACKAGE:
    for my $Package ( @PackagesToVerify ) {

        next PACKAGE if !$Package;

        for my $PackageCheck (@PackageList) {

            # check package name
            if ( $PackageCheck->{Name} eq "$Package" ) {
                $Sum = $PackageCheck->{MD5sum};
            }
        }

        # verify package at web server
        my %VerifyResponse = $WebUserAgentObject->Request(
            URL => $Self->{PackageVerifyURL} . 'Package=' . $Package . '::' . $Sum,
        );
        ...
        my $PackageVerify = $VerifyResponseData->{$Package};
    ...
    for my $Filepath (@Filepaths) {
        ...
        $Kernel::OM->Get('Kernel::System::Log')->Log(
            ...
            Message  => "ERROR: Need write permissions for directory $Home$Filepath\n"
                . " Try: $Home/bin/ofork.SetPermissions.pl!",
        );
...
sub _ConfigurationDeploy {
    ...
    # get OFORK home directory
    my $Home = $Kernel::OM->Get('Kernel::Config')->Get('Home');

    # build file location for OTRS5 config file
    my $OFORK5ConfigFile = "$Home/Kernel/Config/Backups/ZZZAutoOFORK5.pm";

    # if this is a Packageupgrade and if there is a ZZZAutoOFORK5.pm file in the backup location
    if ( ( IsHashRefWithData( $Self->{MergedPackages} ) || $Param{Action} eq 'PackageUpgrade' ) && -e $OFORK5ConfigFile )
    {
        ...
        my $Success = $Kernel::OM->Get('Kernel::System::SysConfig::Migration')->MigrateConfigEffectiveValues(
            FileClass       => 'Kernel::Config::Backups::ZZZAutoOFORK5',
            FilePath        => $OFORK5ConfigFile,
...
sub _ConfiguredRepositoryDefinitionGet {
    ...
    my @Matches = grep { $_ =~ m{https://ftp\.o-fork\.de/pub/itsm/packages\d+/}msxi } sort keys %RepositoryList;
...
#    my $CurrentITSMRepository = "https://ftp.o-fork.de/itsm/packages$FrameworkVersion/";
    my $CurrentITSMRepository = "";
...
#    $RepositoryList{$CurrentITSMRepository} = "OFORK::ITSM $FrameworkVersion Master";
    $RepositoryList{$CurrentITSMRepository} = "";
