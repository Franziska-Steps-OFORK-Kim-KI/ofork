# --
# Kernel/System/DateTime.pm
# Modified version of the work:
# Copyright (C) 2010-2025 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: DateTime.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
Creates a DateTime object. Do not use new() directly, instead use the object manager:
    ...
    # within time zone set in SysConfig OFORKTimeZone:
    ...
    my $DateTimeObject = $Kernel::OM->Create(
        ...
        ObjectParams => {
            ...
            TimeZone => 'Europe/Berlin',        # optional, defaults to setting of SysConfig OFORKTimeZone
        }
    );
...
sub new {
    ...
    #   This has nothing to do with the user's locale settings in OFORK.
    $Self->{Locale} = $Locale;
...
sub ToOFORKTimeZone {
    ...
    return $Self->ToTimeZone( TimeZone => $Self->OFORKTimeZoneGet() );
}
...
Returns the date/time of this object as time stamp in RFC 2822 format to be used in email headers.
    ...
    # Typical usage:
    # You want to have the date/time of OFORK + its UTC offset, so:
...
Checks if date/time of this object is a vacation day.
    ...
    my $IsVacationDay = $DateTimeObject->IsVacationDay(
        Calendar => 9, # optional, OFORK vacation days otherwise
    );
...
=head2 OFORKTimeZoneGet()

Returns the time zone set for OFORK.
...
Returns:
    ...
    my $OFORKTimeZone = 'Europe/Berlin';

=cut

sub OFORKTimeZoneGet {
    return $Kernel::OM->Get('Kernel::Config')->Get('OFORKTimeZone') || 'UTC';
}
...
sub _StringToHash {
    ...
    if ( $Param{String} =~ /^\d{4}-\d{1,2}-\d{1,2}T\d{1,2}:\d{1,2}:\d{1,2}(.+)$/i ) {
        ...
        $DT->set_time_zone( $Self->OFORKTimeZoneGet() );
...
sub _CPANDateTimeObjectCreate {
    ...
    my $TimeZone = $Param{TimeZone} || $Self->OFORKTimeZoneGet();
