# --
# Kernel/System/Ticket/Event/NotificationEvent/Transport/Base.pm
...
returns the specified field with replaced OFORK-tags
    ...
    for example: $RecipientEmail = '<OFORK_TICKET_DynamicField_Name1>';
...
sub _ReplaceTicketAttributes {
    ...
    # replace ticket attributes such as <OFORK_Ticket_DynamicField_Name1> or
    # <OFORK_TICKET_DynamicField_Name1>
    # <OFORK_Ticket_*> is deprecated and should be removed in further versions of OFORK
    my $Count = 0;
    REPLACEMENT:
    while (
        $Param{Field}
        && $Param{Field} =~ m{<OFORK_TICKET_([A-Za-z0-9_]+)>}msxi
        && $Count++ < 1000
        )
    {
        ...
        if ( $TicketAttribute =~ m{DynamicField_(\S+?)_Value} ) {
            ...
            $Param{Field} =~ s{<OFORK_TICKET_$TicketAttribute>}{$DisplayValueStrg->{Value} // ''}ige;
        ...
        if ( !ref $Param{Ticket}->{$TicketAttribute} ) {
            $Param{Field} =~ s{<OFORK_TICKET_$TicketAttribute>}{$Param{Ticket}->{$TicketAttribute} // ''}ige;
        }
        else {
