...
# --
# $Id: NotificationEvent.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub _RecipientsGet {
    ...
    if ( $Notification{Data}->{Recipients} ) {
        ...
        for my $Recipient ( @{ $Notification{Data}->{Recipients} } ) {
            ...
            if (
                $Recipient
                =~ /^Agent(Owner|Responsible|Watcher|WritePermissions|MyQueues|MyServices|MyQueuesMyServices|CreateBy|Supervisor)$/
                )
            {
                ...
                elsif ( $Recipient eq 'Supervisor' ) {

                    my %Queue = $QueueObject->QueueGet(
                        ID => $Ticket{QueueID},
                    );

                    my $SupervisorID = $Queue{SupervisorID};

                    push @{ $Notification{Data}->{RecipientAgents} }, $SupervisorID;
                }
