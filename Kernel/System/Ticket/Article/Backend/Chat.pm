# --
# Kernel/System/Ticket/Article/Backend/Chat.pm
# Modified version of the work:
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: Chat.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub ArticleCreate {
    ...
    for my $ChatMessage ( @{ $Param{ChatMessageList} } ) {
        ...
        my $Success = $DBObject->Do(
            SQL => '
                INSERT INTO article_data_ofork_chat
        ...
        if ( !$Success ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "System was unable to store data in article_data_ofork_chat table (ArticleID = $ArticleID)!",
            );
            return;
        }
    }
...
sub ArticleUpdate {
    ...
    if ( $Param{Key} eq 'ChatMessageList' ) {
        ...
        my $Success = $DBObject->Do(
            SQL => '
                DELETE FROM article_data_ofork_chat
                WHERE article_id = ?
            ',
        ...
        if ( !$Success ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message =>
                    "System was unable to remove data from article_data_ofork_chat table (ArticleID = $Param{ArticleID})!",
            );
            return;
        }
        ...
        for my $ChatMessage ( @{ $Param{Value} } ) {
            ...
            if ( !$Success ) {
                $Kernel::OM->Get('Kernel::System::Log')->Log(
                    Priority => 'error',
                    Message =>
                        "System was unable to store data in article_data_ofork_chat table (ArticleID = $Param{ArticleID})!",
                );
                return;
            }
        }
