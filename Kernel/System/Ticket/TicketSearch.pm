# --
# Kernel/System/Ticket/TicketSearch.pm
# Modified version of the work:
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: TicketSearch.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub TicketSearch {
    ...
    for my $Key (
        ...
        Priorities PriorityIDs CreatedPriorities CreatedPriorityIDs Services ServiceIDs SLAs SLAIDs Process WatchUserIDs
        )
        )
    {
    ...
    if ( $Param{ArticleFlagCustomer} ) {
         $SQLExt = ' WHERE taf1.article_id IS NULL AND ataf1.is_visible_for_customer = 1';
    }
    ...
    if ( $Param{ArticleFlagCustomer} ) {
        my $Index = 1;
        for my $Key ( sort keys %{ $Param{ArticleFlagCustomer} } ) {
            $SQLFrom .= "INNER JOIN article ataf$Index ON st.id = ataf$Index.ticket_id ";
            $SQLFrom .=
                "LEFT JOIN article_flag_customer taf$Index ON ataf$Index.id = taf$Index.article_id ";
            $Index++;
        }
    }
...
#    # add article flag extension
#    if ( $Param{ArticleFlagCustomer} ) {
#        my $ArticleFlagCustomerUserID = $Param{ArticleFlagCustomerUserID} || $Param{UserID};
#        return if !defined $ArticleFlagCustomerUserID;
#
#        my $Index = 1;
#        for my $Key ( sort keys %{ $Param{ArticleFlagCustomer} } ) {
#            my $Value = $Param{ArticleFlagCustomer}->{$Key};
#            return if !defined $Value;
#
#            $SQLExt .= " AND taf$Index.article_key = '" . $DBObject->Quote($Key) . "'";
#            $SQLExt .= " AND taf$Index.article_value = '" . $DBObject->Quote($Value) . "'";
#            $SQLExt .= " AND taf$Index.user_id = '" . $DBObject->Quote($ArticleFlagCustomerUserID) . "'";
#
#            $Index++;
#        }
#    }
    ...
    if ($ArticleTableJoined) {
        ...
        my $OFORKTicketNumber = $Param{OFORKTicketNumber};
        if ( $OFORKTicketNumber ) {
             $SQLExt .= ' OR ( st.tn LIKE "%' . $OFORKTicketNumber . '" ) ';
        }
