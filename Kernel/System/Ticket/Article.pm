...
Since OFORK 7, article data is split in a neutral part for all articles (in the C<article> database table),
...
Events:
    ArticleFlagSetCustomer

=cut

sub ArticleFlagSetCustomer {
    my ( $Self, %Param ) = @_;
    ...
    my %Flag = $Self->ArticleFlagGetCustomer(%Param);
    ...
    return if !$DBObject->Do(
        SQL => '
            DELETE FROM article_flag_customer
            WHERE article_id = ?
                AND article_key = ?
                AND user_id = ?',
        Bind => [ \$Param{ArticleID}, \$Param{Key}, \$Param{UserID} ],
    ...
    return if !$DBObject->Do(
        SQL => 'INSERT INTO article_flag_customer
            (article_id, article_key, article_value, create_time, user_id)
            VALUES (?, ?, ?, current_timestamp, ?)',
        Bind => [ \$Param{ArticleID}, \$Param{Key}, \$Param{Value}, \$Param{UserID} ],
    );
...
=head2 ArticleFlagDeleteCustomer()

Delete an article flag.

    my $Success = $ArticleObject->ArticleFlagDeleteCustomer(
        TicketID  => 123,
        ArticleID => 123,
        Key       => 'seen',
        UserID    => 123,
...
Events:
    ArticleFlagDeleteCustomer

=cut

sub ArticleFlagDeleteCustomer {
    my ( $Self, %Param ) = @_;
    ...
    if ( $Param{AllUsers} ) {
        ...
        # event
        $Self->EventHandler(
            Event => 'ArticleFlagDeleteCustomer',
            Data  => {
                TicketID  => $Param{TicketID},
                ArticleID => $Param{ArticleID},
                Key       => $Param{Key},
...
=head2 ArticleFlagGetCustomer()

Get article flags.

    my %Flags = $ArticleObject->ArticleFlagGetCustomer(
        ArticleID => 123,
        UserID    => 123,
    );
...
=head2 ArticleFlagsOfTicketGetCustomer()

Get all article flags of a ticket.

    my %Flags = $ArticleObject->ArticleFlagsOfTicketGetCustomer(
        TicketID  => 123,
        UserID    => 123,
    );
...
sub ArticleFlagsOfTicketGetCustomer {
    my ( $Self, %Param ) = @_;
    ...
    return if !$DBObject->Prepare(
        SQL => '
            SELECT article.id, article_flag_customer.article_key, article_flag_customer.article_value
            FROM article_flag_customer, article
            WHERE article.id = article_flag_customer.article_id
                AND article.ticket_id = ?
                AND article_flag_customer.user_id = ?',
        Bind  => [ \$Param{TicketID}, \$Param{UserID} ],
        Limit => 1500,
    );
