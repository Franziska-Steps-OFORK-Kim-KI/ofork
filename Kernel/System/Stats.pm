...
# --
# $Id: Stats.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
search parameters, the selection will be converted to the OFORK time zone, because the times
...
the period will be converted from the +6 time zone to the OFORK time zone for the search parameter,
so that the right time will be used for searching the database. Given that the OFORK time zone is set to UTC, this
...
This is the start of the period, which will be extended to 00:00:00. Start and end date will be converted to the time zone of OFORK to search the database.

Example for relative time period 'last 10 full days' with selected time zone offset +10 hours, current date/time within this time zone 2015-09-10 16:00:00, OFORK time zone is UTC:
End date: 2015-09-10 16:00:00 -> extended to 2015-09-10 23:59:59 -> 2015-09-10 13:59:59 OFORK time zone (UTC)
Start date: 2015-09-10 16:00:00 - 10 days -> 2015-08-31 16:00:00 -> extended to 00:00:00: 2015-09-01 00:00:00 -> 2015-08-31 14:00:00 OFORK time zone (UTC)
...
sub StatsGet {
    ...
    my %Stat;
    my $StatsXML = $XMLHash[0]->{ofork_stats}->[1];
    ...
    if ( defined $StatsXML->{TimeZone}->[1]->{Content} ) {
        ...
        if ( Kernel::System::DateTime->IsTimeZoneValid( TimeZone => $StatsXML->{TimeZone}->[1]->{Content} ) ) {
            ...
            $Stat{TimeZone} = Kernel::System::DateTime->OFORKTimeZoneGet();
        }
    }
...
sub GetParams {
    ...
    # static
    # don't remove this if clause, because is required for ofork.GenerateStats.pl
    my @Params;
...
sub StatNumber2StatID {
    ...
    my @Key = $Kernel::OM->Get('Kernel::System::XML')->XMLHashSearch(
        Type => 'Stats',
        What => [ { "[%]{'ofork_stats'}[%]{'StatNumber'}[%]{'Content'}" => $Param{StatNumber} } ],
    );
    if ( @Key && $#Key < 1 ) {
        return $Key[0];
    }
...
sub _GenerateDynamicStats {
    ...
    for my $Use (qw(UseAsRestriction UseAsXvalue UseAsValueSeries)) {
        ...
        for my $Element (@Array) {
            ...
            # Select All function needed from ofork.GenerateStats.pl and fixed values of the frontend
    ...
    for my $RestrictionPart ( @{ $Param{UseAsRestriction} } ) {
        ...
        if ( $RestrictionPart->{Block} eq 'InputField' ) {
            ...
            # convert start and stop time to OFORK time zone
            $RestrictionAttribute{ $RestrictionPart->{Values}{TimeStart} } = $Self->_ToOFORKTimeZone(
                String   => $RestrictionPart->{TimeStart},
                TimeZone => $Param{TimeZone},
            );

            $RestrictionAttribute{ $RestrictionPart->{Values}{TimeStop} } = $Self->_ToOFORKTimeZone(
    ...
    if ( $Element->{Block} eq 'Time' ) {
            ...
            )
        {
            ...
            push(
                ...
                {
                    # convert to OFORK time zone for correct database search parameter

                    TimeStart => $Self->_ToOFORKTimeZone(
                        String   => $TimeStart,
                        TimeZone => $Param{TimeZone},
                    ),

                    TimeStop => $Self->_ToOFORKTimeZone(
                        String   => $TimeStop,
                        TimeZone => $Param{TimeZone},
                    ),
                }
    ...
    # convert to OFORK time zone to get the correct time for the check
    my $CheckTimeStop = $Self->_ToOFORKTimeZone(
...
=head2 _FromOFORKTimeZone()

Converts the given date/time string from OFORK time zone to the given time zone.

    my $String = $StatsObject->_FromOFORKTimeZone(
        String   => '2016-02-20 20:00:00',
        TimeZone => 'Europe/Berlin',
    );

Returns (example for OFORK time zone being set to UTC):

    $TimeStamp = '2016-02-20 21:00:00',

=cut

sub _FromOFORKTimeZone {
    my ( $Self, %Param ) = @_;
...
=head2 _ToOFORKTimeZone()

Converts the given date/time string from the given time zone to OFORK time zone.

    my $String = $StatsObject->_ToOFORKTimeZone(
...
sub _ToOFORKTimeZone {
    ...
    $DateTimeObject->ToOFORKTimeZone();
