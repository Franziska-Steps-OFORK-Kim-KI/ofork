...
# --
# $Id: Selenium.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
perform a get() call, but wait for the page to be fully loaded (works only within OFORK).
...
sub VerifiedGet {
    ...
    ) || die "OFORK API verification failed after page load.";
...
perform a refresh() call, but wait for the page to be fully loaded (works only within OFORK).
Will die() if the verification fails.
...
for analysis (in folder /var/ofork-unittest if it exists, in $Home/var/httpd/htdocs otherwise).
...
sub HandleError {
    ...
    #
    if ( -d '/var/ofork-unittest/' ) {

        my $SharedScreenshotDir = '/var/ofork-unittest/SeleniumScreenshots';
