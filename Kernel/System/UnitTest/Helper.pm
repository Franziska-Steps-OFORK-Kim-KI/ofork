...
# --
# $Id: Helper.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
Disable scheduling of asynchronous tasks using C<AsynchronousExecutor> component of OFORK daemon.
...
Provide temporary database for the test. Please first define test database settings in C<Config.pm>, i.e:
    ...
    $Self->{TestDatabase} = {
        DatabaseDSN  => 'DBI:mysql:database=ofork_test;host=127.0.0.1;',
        DatabaseUser => 'ofork_test',
        DatabasePw   => 'ofork_test',
    };
...
All database contents will be automatically dropped when the Helper object is destroyed.
    ...
    $Helper->ProvideTestDatabase(
        DatabaseXMLString => $XML,      # (optional) OFORK database XML schema to execute
                                        # or
        DatabaseXMLFiles => [           # (optional) List of XML files to load and execute
            '/opt/ofork/scripts/database/ofork-schema.xml',
            '/opt/ofork/scripts/database/ofork-initial_insert.xml',
        ],
    );
...
Execute supplied XML against current database. Content of supplied XML or XMLFilename parameter must be valid OFORK
database XML schema.

    $Helper->DatabaseXMLExecute(
        XML => $XML,     # OFORK database XML schema to execute
    );

Alternatively, it can also load an XML file to execute:

    $Helper->DatabaseXMLExecute(
        XMLFile => '/path/to/file',  # OFORK database XML file to execute
    );
