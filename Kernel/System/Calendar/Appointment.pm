# --
# Kernel/System/Calendar/Appointment.pm
# Modified version of the work:
# Copyright (C) 2010-2025 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: Appointment.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub AppointmentCreate {
    ...
    push @Bind, \$Param{CalendarID}, \$Param{UniqueID}, \$Param{Title}, \$Param{Description},
        ...
        \$Param{TicketAppointmentRuleID}, \$Param{UserID}, \$Param{UserID}, \$Param{QueueID}, \$Param{TicketUserID};
    ...
    my $SQL = "
        INSERT INTO calendar_appointment
            ...
            change_by, queue_id, ticket_user_id)
        VALUES ($ParentIDVal ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
...
sub AppointmentGet {
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        $Result{QueueID}                               = $Row[32];
        $Result{TicketUserID}                          = $Row[33];
    }
...
sub AppointmentUpdate {
    ...
    my $SQL = '
        ...
        SET
            ...
            change_time=current_timestamp, change_by=?, queue_id=?, ticket_user_id=?
        WHERE id=?
    ';
    ...
    return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
        ...
        Bind => [
            ...
            \$Param{NotificationCustomRelativeUnitCount},   \$Param{NotificationCustomRelativeUnit},
            \$Param{NotificationCustomRelativePointOfTime}, \$Param{NotificationCustomDateTime},
            \$Param{TicketAppointmentRuleID}, \$Param{UserID}, \$Param{QueueID}, \$Param{TicketUserID}, \$Param{AppointmentID},
        ],
    );
...
Update OFORK daemon future task list for upcoming appointments.
...
Will be triggered by the OFORK daemon to fire events for appointments,
...
sub AppointmentNotification {
    ...
    for my $UpcomingAppointment (@UpcomingAppointments) {
        ...
        my %User = $UserObject->GetUserData(
            UserID => $Appointment{TicketUserID},
        );
        ...
        my $TicketID = $TicketObject->TicketCreate(
            ...
            Priority     => '3 normal',
            State        => 'new',
            CustomerID   => $User{UserEmail},
            CustomerUser => $User{UserEmail},
            OwnerID      => $Appointment{TicketUserID},
            UserID       => $Appointment{TicketUserID},
        );
        ...
        my $ArticleID = $ArticleBackendObject->ArticleCreate(
            ...
            HistoryComment       => 'New calendar ticket' || '%%',
            AutoResponseType => '',
            OrigHeader => {
                From    => $User{UserEmail},
                To      => $User{UserLogin},
            ...
            Queue => $QueueObject->QueueLookup( QueueID => $Appointment{QueueID} ),
        );
