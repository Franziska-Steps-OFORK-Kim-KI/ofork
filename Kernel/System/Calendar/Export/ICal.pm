# --
# Kernel/System/Calendar/Export/ICal.pm
...
sub Export {
    ...
    for my $AppointmentID (@AppointmentIDs) {
        ...
        for my $PluginKey ( sort keys %{$PluginList} ) {
            ...
            if (@LinkArray) {
                $ICalEvent->add_properties(
                    "x-ofork-plugin-$PluginKey" => join( ',', @LinkArray ),
                );
            }
        }
...

{
    ...
    sub Data::ICal::product_id {
        return 'OFORK ' . $Kernel::OM->Get('Kernel::Config')->Get('Version');
    }
}
