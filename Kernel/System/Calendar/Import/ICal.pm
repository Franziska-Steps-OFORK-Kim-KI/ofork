# --
# Kernel/System/Calendar/Import/ICal.pm
# Modified version of the work:
# Copyright (C) 2010-2025 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: ICal.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Import {
    ...
    # Prevent line ending type errors
    $Param{ICal} =~ s/\r/\n/g;
    ...
    for my $Entry (@Entries) {
            ...
            )
        {
            ...
            if ( !$Parameters{AllDay} ) {
                $StartTimeObject->ToOFORKTimeZone();
            }
            ...
            if ( IsArrayRefWithData( $Properties->{'exdate'} ) ) {
                ...
                for my $Exclude ( @{ $Properties->{'exdate'} } ) {
                        ...
                        )
                    {
                        ...
                        if ( !$Parameters{AllDay} ) {
                            $ExcludeTimeObject->ToOFORKTimeZone();
                        }
        ...
        if ( $Kernel::OM->Get('Kernel::System::Main')->Require( 'Kernel::System::Calendar::Team', Silent => 1 ) ) {
            ...
            # get team
            if (
                IsArrayRefWithData( $Properties->{'x-ofork-team'} )
                && ref $Properties->{'x-ofork-team'}->[0] eq 'Data::ICal::Property'
                && $Properties->{'x-ofork-team'}->[0]->{'value'}
                )
            {
                my @Teams = split( ",", $Properties->{'x-ofork-team'}->[0]->{'value'} );
            ...
            # get resource
            if (
                IsArrayRefWithData( $Properties->{'x-ofork-resource'} )
                && ref $Properties->{'x-ofork-resource'}->[0] eq 'Data::ICal::Property'
                && $Properties->{'x-ofork-resource'}->[0]->{'value'}
                )
            {
                my @Resources = split( ",", $Properties->{'x-ofork-resource'}->[0]->{'value'} );
        ...
        # plugin fields (start with 'x-ofork-plugin-')
        my @PluginFields = grep { $_ =~ /x-ofork-plugin-/i } keys %{$Properties};
            ...
            )
        {
            ...
            if ( !$Parameters{AllDay} ) {
                $RecurrenceIDObject->ToOFORKTimeZone();
            }
