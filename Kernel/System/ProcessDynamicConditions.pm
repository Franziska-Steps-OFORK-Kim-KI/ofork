# --
# Kernel/System/ProcessDynamicConditions.pm
# Copyright (C) 2010-2025 OFORK, https://o-fork.de
# --
# $Id: ProcessDynamicConditions.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
package Kernel::System::ProcessDynamicConditions;
...
Kernel::System::ProcessDynamicConditions - ProcessDynamicConditions lib

=head1 DESCRIPTION

All Equipment functions. E. g. to add ProcessDynamicConditions.
...
sub ProcessDynamicConditionsAdd {
    ...
    # check needed stuff
    for my $Needed (qw(ProcessID ProcessStepID DynamicFieldID DynamicValue UserID)) {
        if ( !$Param{$Needed} ) {
    ...
    return if !$DBObject->Do(
        SQL => 'INSERT INTO process_d_conditions (process_id, processstep_id, dynamicfield_id, dynamicfield_value,'
            . ' create_time, create_by, change_time, change_by)'
            . ' VALUES (?, ?, ?, ?, current_timestamp, ?, current_timestamp, ?)',
        Bind => [
            \$Param{ProcessID}, \$Param{ProcessStepID}, \$Param{DynamicFieldID}, \$Param{DynamicValue}, \$Param{UserID}, \$Param{UserID},
        ],
    );
...
sub ProcessDynamicConditionsGet {
    ...
    return if !$DBObject->Prepare(
        SQL => 'SELECT id, process_id, processstep_id, dynamicfield_id, dynamicfield_value, create_time, create_by, change_time, change_by '
            . 'FROM process_d_conditions WHERE id = ?',
...
sub ProcessDynamicConditionsList {
    ...
    return if !$DBObject->Prepare(
        SQL => 'SELECT id, processstep_id FROM process_d_conditions WHERE process_id = ? AND processstep_id = ?',
