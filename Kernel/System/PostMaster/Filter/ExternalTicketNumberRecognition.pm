# --
# Kernel/System/PostMaster/Filter/ExternalTicketNumberRecognition.pm
# Modified version of the work:
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: ExternalTicketNumberRecognition.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    if ($TicketID) {
        ...
        $Param{GetParam}->{'X-OFORK-FollowUp-SenderType'}           = $Param{JobConfig}->{SenderType};
        $Param{GetParam}->{'X-OFORK-FollowUp-IsVisibleForCustomer'} = $Param{JobConfig}->{IsVisibleForCustomer};
        ...
        $Param{GetParam}->{'X-OFORK-SenderType'}           = $Param{JobConfig}->{SenderType};
        $Param{GetParam}->{'X-OFORK-IsVisibleForCustomer'} = $Param{JobConfig}->{IsVisibleForCustomer};
        ...
        $Param{GetParam}->{ 'X-OFORK-DynamicField-' . $TicketDynamicFieldName } = $Self->{Number};
