# --
# Kernel/System/PostMaster/Filter/FollowUpArticleVisibilityCheck.pm
# Modified version of the work:
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: FollowUpArticleVisibilityCheck.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    if (
        !$Param{GetParam}->{'X-OFORK-FollowUp-SenderType'}
        || $Param{GetParam}->{'X-OFORK-FollowUp-SenderType'} ne 'customer'
        )
    {
        return 1;
    }
    ...
    $Param{GetParam}->{'X-OFORK-FollowUp-IsVisibleForCustomer'} = $Param{JobConfig}->{IsVisibleForCustomer} // 0;
    $Param{GetParam}->{'X-OFORK-FollowUp-SenderType'} = $Param{JobConfig}->{SenderType} || 'customer';
