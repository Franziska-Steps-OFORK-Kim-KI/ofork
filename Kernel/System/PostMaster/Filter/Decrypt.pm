...
# --
# $Id: Decrypt.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    if ( $EncryptionMethod eq 'PGP' ) {
        ...
        $Param{GetParam}->{'X-OFORK-BodyDecrypted'} = $Self->_DecryptPGP(
            Body        => $Message,
        ...
        return $Param{GetParam}->{'X-OFORK-BodyDecrypted'} if $Param{GetParam}->{'X-OFORK-BodyDecrypted'};
    }
    elsif ( $EncryptionMethod eq 'SMIME' ) {

        # Try to decrypt body with SMIME.
        $Param{GetParam}->{'X-OFORK-BodyDecrypted'} = $Self->_DecryptSMIME(
        ...
        $Param{GetParam}->{'X-OFORK-BodyDecrypted'} = '';
    }
