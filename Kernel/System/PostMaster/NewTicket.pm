...
# --
# $Id: NewTicket.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    if ( $GetParam{'X-OFORK-State'} ) {
        ...
        if ($StateID) {
            ...
            $Self->{CommunicationLogObject}->ObjectLog(
                ...
                Priority      => 'Error',
                Key           => 'Kernel::System::PostMaster::NewTicket',
                Value         => "State $GetParam{'X-OFORK-State'} does not exist, falling back to $State!",
            );
        }
    }
    ...
    if ( $GetParam{'X-OFORK-Priority'} ) {
        ...
        if ($PriorityID) {
            ...
            $Self->{CommunicationLogObject}->ObjectLog(
                ...
                Priority      => 'Error',
                Key           => 'Kernel::System::PostMaster::NewTicket',
                Value         => "Priority $GetParam{'X-OFORK-Priority'} does not exist, falling back to $Priority!",
            );
        }
    }
    ...
    if ( $GetParam{'X-OFORK-Type'} ) {

        # Check if type exists
        $TypeID = $Kernel::OM->Get('Kernel::System::Type')->TypeLookup( Type => $GetParam{'X-OFORK-Type'} );
        ...
        if ( !$TypeID ) {
            $Self->{CommunicationLogObject}->ObjectLog(
                ...
                Priority      => 'Error',
                Key           => 'Kernel::System::PostMaster::NewTicket',
                Value         => "Type $GetParam{'X-OFORK-Type'} does not exist, falling back to default type.",
            );
        }
    }
    ...
    if ( !$GetParam{'X-OFORK-CustomerNo'} && $GetParam{'X-OFORK-CustomerUser'} ) {

        # get customer user object
        my $CustomerUserObject = $Kernel::OM->Get('Kernel::System::CustomerUser');

        # get customer user data form X-OFORK-CustomerUser
        my %CustomerData = $CustomerUserObject->CustomerUserDataGet(
            User => $GetParam{'X-OFORK-CustomerUser'},
        );

        if (%CustomerData) {
            $GetParam{'X-OFORK-CustomerNo'} = $CustomerData{UserCustomerID};
        }
    }

    # get customer user data form From: (sender address)
    if ( !$GetParam{'X-OFORK-CustomerUser'} ) {
        ...
        if ( $CustomerData{UserLogin} && !$GetParam{'X-OFORK-CustomerUser'} ) {
            $GetParam{'X-OFORK-CustomerUser'} = $CustomerData{UserLogin};
        ...
        if ( $CustomerData{UserCustomerID} && !$GetParam{'X-OFORK-CustomerNo'} ) {
            $GetParam{'X-OFORK-CustomerNo'} = $CustomerData{UserCustomerID};
    ...
    if (
        !$GetParam{'X-OFORK-CustomerNo'}
        && $ConfigObject->Get('PostMaster::NewTicket::AutoAssignCustomerIDForUnknownCustomers')
        )
    {
        $GetParam{'X-OFORK-CustomerNo'} = $GetParam{SenderEmailAddress};
    }

    # if there is no customer user found!
    if ( !$GetParam{'X-OFORK-CustomerUser'} ) {
        $GetParam{'X-OFORK-CustomerUser'} = $GetParam{SenderEmailAddress};
    }

    # get ticket owner
    my $OwnerID = $GetParam{'X-OFORK-OwnerID'} || $Param{InmailUserID};
    if ( $GetParam{'X-OFORK-Owner'} ) {

        my $TmpOwnerID = $Kernel::OM->Get('Kernel::System::User')->UserLookup(
            UserLogin => $GetParam{'X-OFORK-Owner'},
        );
    ...
    my %Opts;
    if ( $GetParam{'X-OFORK-ResponsibleID'} ) {
        $Opts{ResponsibleID} = $GetParam{'X-OFORK-ResponsibleID'};
    }

    if ( $GetParam{'X-OFORK-Responsible'} ) {

        my $TmpResponsibleID = $Kernel::OM->Get('Kernel::System::User')->UserLookup(
            UserLogin => $GetParam{'X-OFORK-Responsible'},
        );
    ...
    if ( $GetParam{'X-OFORK-Service'} ) {
        my $ServiceObject = $Kernel::OM->Get('Kernel::System::Service');

        # Check if service exists.
        my %ServiceData = $ServiceObject->ServiceGet(
            Name   => $GetParam{'X-OFORK-Service'},
        ...
        if ( $ServiceData{ServiceID} ne '' && !$ServiceList{ $ServiceData{ServiceID} } ) {
            $Self->{CommunicationLogObject}->ObjectLog(
                ...
                Key           => 'Kernel::System::PostMaster::NewTicket',
                Value =>
                    "Service $GetParam{'X-OFORK-Service'} does not exists or is invalid or is a child of invalid service.",
            );
            $GetParam{'X-OFORK-Service'} = '';
        }
    }
    ...
    my $TicketID = $TicketObject->TicketCreate(
        TN           => $NewTn,
        Title        => $GetParam{'X-OFORK-Title'} || $GetParam{Subject},
        QueueID      => $QueueID,
        Lock         => $GetParam{'X-OFORK-Lock'} || 'unlock',
        Priority     => $Priority,
        State        => $State,
        TypeID       => $TypeID,
        Service      => $GetParam{'X-OFORK-Service'} || '',
        SLA          => $GetParam{'X-OFORK-SLA'} || '',
        CustomerID   => $GetParam{'X-OFORK-CustomerNo'},
        CustomerUser => $GetParam{'X-OFORK-CustomerUser'},
        OwnerID      => $OwnerID,
        UserID       => $Param{InmailUserID},
        %Opts,
    );
...
Message
    ...
    for my $Value (qw(Type Service SLA Lock)) {
        ...
        if ( $GetParam{ 'X-OFORK-' . $Value } ) {
            $TicketCreateMessage .= "$Value: " . $GetParam{ 'X-OFORK-' . $Value } . "\n";
        }
    }
    ...
    # set pending time
    if ( $GetParam{'X-OFORK-State-PendingTime'} ) {
  ...
  # "+12d" (12 days). Note that settings like "+1d 12h" are not possible. You can specify "+36h" instead.
        ...
        my $TargetTimeStamp = $GetParam{'X-OFORK-State-PendingTime'};
        ...
        $Self->{CommunicationLogObject}->ObjectLog(
            ...
            Key           => 'Kernel::System::PostMaster::NewTicket',
            Value =>
                "Pending time update via 'X-OFORK-State-PendingTime'! State-PendingTime: $GetParam{'X-OFORK-State-PendingTime'}.",
        );
    }
    ...
    for my $DynamicFieldID ( sort keys %{$DynamicFieldList} ) {
        ...
        my $Key = 'X-OFORK-DynamicField-' . $DynamicFieldList->{$DynamicFieldID};
    ...
    my %Values =
        (
        'X-OFORK-TicketKey'   => 'TicketFreeKey',
        'X-OFORK-TicketValue' => 'TicketFreeText',
        );
    ...
    for my $Count ( 1 .. 6 ) {
        ...
        my $Key = 'X-OFORK-TicketTime' . $Count;
    ...
    if ( length $GetParam{'X-OFORK-IsVisibleForCustomer'} ) {
        $IsVisibleForCustomer = $GetParam{'X-OFORK-IsVisibleForCustomer'};
    }
    ...
    my $ArticleID = $ArticleBackendObject->ArticleCreate(
        TicketID             => $TicketID,
        SenderType           => $GetParam{'X-OFORK-SenderType'},
    ...
    if ( !$ArticleID ) {
        ...
        $Self->{CommunicationLogObject}->ObjectLog(
            ...
            Value         => "Can't process email with MessageID <$GetParam{'Message-ID'}>! "
                ...
                . "under var/spool/problem-email*) on https://bugs.o-fork.de/!",
        );
    ...
    %Values =
        (
        'X-OFORK-ArticleKey'   => 'ArticleFreeKey',
        'X-OFORK-ArticleValue' => 'ArticleFreeText',
        );
    for my $Item ( sort keys %Values ) {
        for my $Count ( 1 .. 16 ) {
            my $Key = $Item . $Count;
