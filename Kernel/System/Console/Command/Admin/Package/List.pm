# --
# Kernel/System/Console/Command/Admin/Package/List.pm
...
sub Configure {
    ...
    $Self->Description('List all installed OFORK packages.');
    ...
    $Self->AddOption(
        ...
        Description => 'Show package OFORK Verify status.',
        Required    => 0,
        HasValue    => 0,
    );
    ...
    $Self->AddOption(
        ...
        Description => 'Delete OFORK Verify cache, so verification info is fetch again from OFORK group servers.',
        Required    => 0,
        HasValue    => 0,
    );
...
sub Run {
    ...
    for my $Package (@Packages) {
        ...
        if ($ShowVerificationInfoOption) {
            ...
            {
                $Self->Print("| <red>OFORK Verify:</red> Not Verified\n");
            }
            else {
                $Self->Print("| <yellow>OFORK Verify:</yellow> Verified\n");
            }
        }
    }
    $Self->Print("+----------------------------------------------------------------------------+\n");
