# --
# Kernel/Modules/AdminCustomerUserRequestGroup.pm
# Modified version of the work:
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: AdminCustomerUserRequestGroup.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
package Kernel::Modules::AdminCustomerUserRequestGroup;
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'CustomerUser' ) {
        ...
        # get group data
        my %GroupData = $RequestGroupObject->GroupList( Valid => 1 );
        my %Types;
        for my $Type ( @{ $ConfigObject->Get('System::Customer::RequestPermission') } ) {
            my %Data = $CustomerRequestGroupObject->GroupMemberList(
                UserID         => $ID,
                Type           => $Type,
                Result         => 'HASH',
                RawPermissions => 1,
    ...
    # ------------------------------------------------------------ #
        ...
        # get group data
        my $ID = $ParamObject->GetParam( Param => 'ID' );
        my %GroupData = $RequestGroupObject->GroupGet( ID => $ID );
    ...
    # ------------------------------------------------------------ #
        ...
        for my $UserID ( sort keys %UserData ) {
            ...
            $CustomerRequestGroupObject->GroupMemberAdd(
                UID        => $UserID,
                GID        => $ID,
...
sub _Change {
    ...
    return $LayoutObject->Output(
        TemplateFile => 'AdminCustomerUserRequestGroup',
        Data         => \%Param,
    );
}
