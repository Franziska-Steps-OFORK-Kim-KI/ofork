# --
# Kernel/Modules/AdminRequestGroup.pm
# Modified version of the work:
# Copyright (C) 2010-2025 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: AdminRequestGroup.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
package Kernel::Modules::AdminRequestGroup;
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'Change' ) {
        my $ID = $ParamObject->GetParam( Param => 'ID' )
            ...
            || '';
        my %Data = $RequestGroupObject->GroupGet( ID => $ID );
        ...
        $Output .= $LayoutObject->Output(
            TemplateFile => 'AdminRequestGroup',
            Data         => \%Param,
        );
    ...
    # ------------------------------------------------------------ #
        ...
        if ( !%Errors ) {
            ...
            # update group
            my $GroupUpdate = $RequestGroupObject->GroupUpdate(
                %GetParam,
                UserID => $Self->{UserID}
            );
    ...
    # ------------------------------------------------------------ #
        ...
        if ( !%Errors ) {
            ...
            # add group
            $GroupID = $RequestGroupObject->GroupAdd(
                %GetParam,
                UserID => $Self->{UserID}
            );
...
sub _Overview {
    ...
    my %List = $RequestGroupObject->GroupList(
        ValidID => 0,
    );
    ...
    for my $ListKey ( sort { $List{$a} cmp $List{$b} } keys %List ) {
        ...
        my %Data = $RequestGroupObject->GroupGet(
            ID => $ListKey,
        );
        $LayoutObject->Block(
