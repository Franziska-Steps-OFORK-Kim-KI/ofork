...
# --
# $Id: Installer.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    # Print intro form.
    my $Title = $LayoutObject->{LanguageObject}->Translate('Install OFORK');
    ...
    # Check different requirements (AJAX).
        ...
        if ( $CheckMode eq 'DB' ) {
            my %DBCredentials;
            for my $Param (
                qw(DBUser DBPassword DBHost DBType DBPort DBSID DBName InstallType OFORKDBUser OFORKDBPassword)
                )
            {
        ...
        if ( $DBType eq 'mysql' ) {
            ...
            $LayoutObject->Block(
                ...
                Data => {
                    ...
                    DefaultDBUser       => $DBInstallType eq 'CreateDB' ? 'root' : 'ofork',
            ...
            $LayoutObject->Block(
                ...
                Data => {
                    ...
                    Step          => $StepCounter,
                    InstallType   => $DBInstallType,
                    DefaultDBUser => $DBInstallType eq 'CreateDB' ? 'postgres' : 'ofork',
                },
            );
            if ( $DBInstallType eq 'CreateDB' ) {
                $LayoutObject->Block(
    ...
    # Do database settings.
        ...
        my %DBCredentials;
        for my $Param (
            qw(DBUser DBPassword DBHost DBType DBName DBSID DBPort InstallType OFORKDBUser OFORKDBPassword)
            )
        {
            $DBCredentials{$Param} = $ParamObject->GetParam( Param => $Param ) || '';
        }
        ...
        if ( $DB{DBType} eq 'mysql' ) {
            ...
            if ( $DB{InstallType} eq 'CreateDB' ) {
                ...
                @Statements = (
                    ...
                    "GRANT ALL PRIVILEGES ON `$DB{DBName}`.* TO `$DB{OFORKDBUser}`\@`$DB{Host}` IDENTIFIED BY '$DB{OFORKDBPassword}' WITH GRANT OPTION",
                    "FLUSH PRIVILEGES",
                );
            }
            ...
            if ( $DB{InstallType} eq 'CreateDB' ) {
                @Statements = (
                    "CREATE ROLE \"$DB{OFORKDBUser}\" WITH LOGIN PASSWORD '$DB{OFORKDBPassword}'",
                    "CREATE DATABASE \"$DB{DBName}\" OWNER=\"$DB{OFORKDBUser}\" ENCODING 'utf-8'",
                );
            }
        ...
        if ( $DB{DBType} eq 'oracle' ) {
            $ReConfigure = $Self->ReConfigure(
                ...
                Database     => $DB{DBSID},
                DatabaseUser => $DB{OFORKDBUser},
                DatabasePw   => $DB{OFORKDBPassword},
            );
        }
        else {
            $ReConfigure = $Self->ReConfigure(
        ...
        if ($ReConfigure) {
            my $Output =
                $LayoutObject->Header(
                Title => Translatable('Install OFORK - Error')
                );
            $Output .= $LayoutObject->Warning(
                Message => Translatable('Kernel/Config.pm isn\'t writable!'),
                Comment => Translatable(
        ...
        my @SQLPost;
        for my $SchemaFile (qw(ofork-schema ofork-initial_insert)) {
            ...
            @SQLPost = $DBObject->SQLProcessorPost() if $SchemaFile eq 'ofork-schema';

            my $SQLNum = 0;
            for my $SQL (@SQL) {
                $SQLNum ++;
                print STDERR "Create Database. Step: $SQLNum\n";
                $DBObject->Do( SQL => $SQL );
            }
        ...
        for my $SQL (@SQLPost) {
            $SQLPostNum ++;
            print STDERR "Set Database Inserts. Step: $SQLPostNum\n";
            $DBObject->Do( SQL => $SQL );
        }
    ...
    # Do system settings action.
        ...
        my $NewDirectory = $ConfigObject->Get('Home') . '/var/packages';
        if ( !-e $NewDirectory ) {
            if ( mkdir $NewDirectory ) {
                print STDERR "Notice: Create Directory $NewDirectory!\n";
            }
        }
        ...
        my $OFORKHandle = $ENV{SCRIPT_NAME};
        $OFORKHandle =~ s/\/(.*)\/installer\.pl/$1/;
        my $Output =
            $LayoutObject->Header(
            Title => "$Title - "
...
sub ConnectToDB {
    ...
    if ( $Param{InstallType} eq 'CreateDB' ) {
        push @NeededKeys, qw(OFORKDBUser OFORKDBPassword);
    }
    ...
    # If we do not need to create a database for OFORK OFORKDBuser equals DBUser.
    if ( $Param{InstallType} ne 'CreateDB' ) {
        $Param{OFORKDBUser}     = $Param{DBUser};
        $Param{OFORKDBPassword} = $Param{DBPassword};
    }
