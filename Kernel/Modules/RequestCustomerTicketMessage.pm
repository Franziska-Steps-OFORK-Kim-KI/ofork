# --
# Kernel/Modules/RequestCustomerTicketMessage.pm - to handle customer messages
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# --
# $Id: RequestCustomerTicketMessage.pm,v 1.74 2016/12/13 14:38:03 ud Exp $
# --
...
package Kernel::Modules::RequestCustomerTicketMessage;
...
sub Run {
    ...
    for my $Key (
        qw( Subject Body PriorityID TypeID ServiceID SLAID Expand Dest FromChatID RequestID RequestFormIDs RequestFormBlockIDs BodyAlt)
        )
    {
    ...
    if ( $Request{SubjectChangeable} && $Request{SubjectChangeable} == 1 ) {
...
sub _MaskNew {
    ...
    return $LayoutObject->Output(
        TemplateFile => 'RequestCustomerTicketMessage',
        Data         => \%Param,
    );
}
...
sub _SetBlockHTML {
    ...
    if ( $GetParam{RequestFormBlockIDs} ) {
        ...
        $ArticleBody .= '<hr style="background:#000000;height:1px;" />';
...
sub _MaskNewNoValid {
    ...
    # get output back
    return $LayoutObject->Output(
        TemplateFile => 'RequestCustomerTicketMessageNoValid',
        Data         => \%Param,
    );
}
