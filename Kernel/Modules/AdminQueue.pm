...
# --
# $Id: AdminQueue.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    # ------------------------------------------------------------ #
        ...
        for my $Optional (
            qw(UnlockTimeout FirstResponseTime FirstResponseNotify SupervisorID UpdateTime UpdateNotify SolutionTime SolutionNotify FollowUpLock Calendar)
            )
        {
...
sub _Edit {
    ...
    delete $UserList{1};
    $Param{SupervisorIDOption} = $LayoutObject->BuildSelection(
        Data          => \%UserList,
        Name          => 'SupervisorID',
        SelectedID    => $Param{SupervisorID},
        PossibleNone  => 1,
        Class         => 'Modernize',
    );
