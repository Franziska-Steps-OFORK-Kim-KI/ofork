...
# --
# $Id: AgentTicketCompose.pm,v 1.1.1.1 2020/02/20 06:48:16 ud Exp $
# --
...
sub Run {
    ...
    for (
        qw(
        From To Cc Bcc Subject Body InReplyTo References ResponseID ReplyArticleID RequestFormIDs StateID ArticleID
    ...
    if ( $Self->{Subaction} eq 'SendEmail' || $Self->{LoadedFormDraftID} ) {
        ...
        # replace <OFORK_TICKET_STATE> with next ticket state name
        if ( $StateData{Name} ) {
            $GetParam{Body} =~ s/<OFORK_TICKET_STATE>/$StateData{Name}/g;
            $GetParam{Body} =~ s/&lt;OFORK_TICKET_STATE&gt;/$StateData{Name}/g;
        }
        ...
        my $ArticleBody = '<br><br><div id="Request">';
    ...
    # check for SMIME / PGP if customer has changed
        ...
        my $RequestFieldsObject = $Kernel::OM->Get('Kernel::System::RequestFields');
        my %RequestFormList = $RequestFieldsObject->RequestFieldsStandardTemplateMemberList(
            StandardTemplateID => $GetParam{ResponseID},
        );
...
sub _Mask {
    ...
    if ( $Param{ResponseID} ) {
        ...
        my %RequestFormList = $RequestFieldsObject->RequestFieldsStandardTemplateMemberList(
            StandardTemplateID => $Param{ResponseID},
        );

        my $IfValue;
        for my $RequestFormListID ( sort { $RequestFormList{$a} <=> $RequestFormList{$b} } keys %RequestFormList ) {
            ...
            #get RequestField
            my %RequestFields = $RequestFieldsObject->RequestFieldsGet(
                RequestFieldsID => $RequestFormListID,
            );
            ...
            if ( $RequestFields{Typ} eq "Dropdown" ) {
            ...
            if ( $RequestFields{Typ} eq "Checkbox" ) {
