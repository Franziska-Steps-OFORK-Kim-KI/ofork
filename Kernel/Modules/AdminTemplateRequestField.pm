# --
# Kernel/Modules/AdminTemplateRequestField.pm
# Modified version of the work:
# Copyright (C) 2010-2024 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: AdminTemplateRequestField.pm,v 1.1.1.1 2020/02/20 06:48:16 ud Exp $
# --
...
package Kernel::Modules::AdminTemplateRequestField;
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'Template' ) {
        ...
        # get request fields data
        my %RequestFieldsData = $RequestFieldsObject->RequestFieldsAdminList( Valid => 1, UserID => 1, );

        my %Member = $RequestFieldsObject->RequestFieldsStandardTemplateMemberList(
    ...
    # ------------------------------------------------------------ #
    # request field <-> template n:1
    # ------------------------------------------------------------ #
    elsif ( $Self->{Subaction} eq 'RequestField' ) {

        # get group data
        my $ID = $ParamObject->GetParam( Param => 'ID' );
        my %RequestFieldData = $RequestFieldsObject->RequestFieldsGet( RequestFieldsID => $ID );
        ...
        $Output .= $Self->_Change(
            ...
            ID       => $RequestFieldData{ID},
            Name     => $RequestFieldData{Name},
            Type     => 'RequestField',
        );
        $Output .= $LayoutObject->Footer();
        return $Output;
    ...
    # add user to groups
    # ------------------------------------------------------------ #
    elsif ( $Self->{Subaction} eq 'ChangeRequestField' ) {
        ...
        my $RequestFieldID = $ParamObject->GetParam( Param => 'ID' );
            ...
            )
        {
            return $LayoutObject->Redirect(
                OP => "Action=$Self->{Action};Subaction=RequestField;ID=$RequestFieldID;Notification=Update"
            );
        }
        else {
...
sub _Change {
    ...
    my $NeType = $Type eq 'RequestField' ? 'Template' : 'RequestField';

    my %VisibleType = (
        Template   => Translatable('Template'),
        Attachment => Translatable('RequestField'),
    );

    my $LayoutObject = $Kernel::OM->Get('Kernel::Output::HTML::Layout');

    my $BreadcrumbTitle = $LayoutObject->{LanguageObject}->Translate('Change RequestField Relations for Template');

    if ( $VisibleType{$Type} eq 'RequestField' ) {
        $BreadcrumbTitle = $LayoutObject->{LanguageObject}->Translate('Change Template Relations for RequestField');
    }
    ...
    return $LayoutObject->Output(
        TemplateFile => 'AdminTemplateRequestField',
        Data         => \%Param,
    );
}
...
sub _Overview {
    ...
    # get queue data
    my %StdAttachmentData = $Kernel::OM->Get('Kernel::System::RequestFields')->RequestFieldsAdminList( Valid => 1, UserID => 1, );
