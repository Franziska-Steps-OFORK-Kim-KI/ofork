...
# --
# $Id: CustomerTicketOverview.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    my @SetTicketType;
    my %TicketTypeList = $TypeObject->TypeList(
        Valid => 0,
    );
    for my $TicketTypeID ( keys %TicketTypeList ) {
        if ( $TicketTypeList{$TicketTypeID} ne "RoomBooking" ) {
            push @SetTicketType, $TicketTypeList{$TicketTypeID};
        }
    }
        ...
        MyTickets => {
            ...
            New => {
                ...
                Search => {
                    ...
                    CustomerUserID            => $Self->{UserID},
                    Permission                => 'ro',
                    ArticleFlagCustomerUserID => $Self->{UserID},
                    ArticleFlagCustomer       => {
                        Seen => 1,
                    },
                },
...
sub ShowTicketStatus {
    ...
    my $CheckFlag = 0;
    for my $IfFlag ( keys %Flags ) {
        $CheckFlag ++;
    }

    if ( $CheckFlag >= 1 ) {

        my %ArticleFlags = $ArticleObject->ArticleFlagsOfTicketGetCustomer(
            TicketID  => $TicketID,
            UserID    => $Self->{UserID},
        );
        ...
        for my $IfArticleFlag ( keys %ArticleFlags ) {
            $ArticleNum ++;
        }
        ...
        if ( $CheckArticleFlag == 0 || $CheckArticleFlag > $ArticleNum ) {

            # add block
            $LayoutObject->Block(
                Name => 'RecordArticleNoSeen',
                Data => {
                    %Article,
                    %Ticket,
                    %Param,
