...
# --
# $Id: AgentAppointmentEdit.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'EditMask' ) {
        ...
        $Param{QueueIDStrg} = $LayoutObject->BuildSelection(
            Data         => \%QueueList,
            Name         => 'QueueID',
            Class        => '',
            SelectedID   => $Appointment{QueueID},
            PossibleNone => 1,
            Sort         => 'NumericValue',
            Translation  => 1,
        );
        ...
        my $UserObject = $Kernel::OM->Get('Kernel::System::User');
        my %TicketUserList = $UserObject->UserList(
            Type  => 'Long',
            Valid => 1,
        );
        $Param{TicketUserIDStrg} = $LayoutObject->BuildSelection(
            Data         => \%TicketUserList,
            Name         => 'TicketUserID',
            Class        => '',
            SelectedID   => $Appointment{TicketUserID},
            PossibleNone => 1,
            Sort         => 'NumericValue',
            Translation  => 1,
        );
    ...
    # ------------------------------------------------------------ #
        ...
        if ( defined $GetParam{NotificationTemplate} ) {
            if ( $GetParam{NotificationTemplate} ne 'Custom' ) {
                ...
                if ( $Self->{UserTimeZone} ) {
                    $NotificationCustomDateTimeObject->ToOFORKTimeZone();
                }
