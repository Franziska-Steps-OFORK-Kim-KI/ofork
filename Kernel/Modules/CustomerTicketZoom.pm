...
# --
# $Id: CustomerTicketZoom.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    for my $ArticleMetaData (@ArticleList) {
        ...
        my $Success = $ArticleObject->ArticleFlagSetCustomer(
            TicketID  => $Self->{TicketID},
            ArticleID => $ArticleMetaData->{ArticleID},
            Key       => 'Seen',
            Value     => 1,
...
sub _Mask {
    ...
    $Param{CustomerTimeUnits} = $TimeTrackingTimeValue;

    # ticket accounted time
    if ( $Config->{CustomerTimeTracking} ) {
        $LayoutObject->Block(
            Name => 'CustomerTimeUnits',
            Data => \%Param,
        );
    }
