...
# --
# $Id: AgentTicketZoom.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub new {
    ...
    $Self->{ChecklistWidget} = $ParamObject->GetParam( Param => 'ChecklistWidget' );
    $Self->{ProcessError}    = $ParamObject->GetParam( Param => 'ProcessError' );
...
sub MaskAgentZoom {
    ...
    my $TimeTrackingAccess = $LayoutObject->Permission(
        Action => 'AgentTicketTimeTracking',
        Type   => 'rw',
    );
    ...
    if ( ref $ConfigObject->Get('Ticket::Frontend::MenuModule') eq 'HASH' ) {
        ...
        for my $Menu ( sort keys %Menus ) {
            ...
            next MENU if !$TimeTrackingAccess && $Menu eq "820-TimeTracking";
    ...
    my $CheckIfQueueID   = 0;
    my $CheckIfTypeID    = 0;
    my $CheckIfServiceID = 0;
    for my $ChecklistID ( keys %ChecklistList ) {
        ...
        my @QueueIDs = ();
        if ( $Checklist{QueueIDs} ) {
            @QueueIDs = split( /,/, $Checklist{QueueIDs} );
        ...
        my @TypeIDs = ();
        if ( $Checklist{TypeIDs} ) {
            @TypeIDs = split( /,/, $Checklist{TypeIDs} );
            for my $TypeID (@TypeIDs  ) {
                if ( $TypeID == $Ticket{TypeID} ) {
                    $CheckIfTypeID = 1;
                }
            }
        }
        ...
        my @ServiceIDs = ();
        if ( $Checklist{ServiceIDs} ) {
            @ServiceIDs = split( /,/, $Checklist{ServiceIDs} );
            for my $ServiceID (@ServiceIDs  ) {
                if ( $ServiceID == $Ticket{ServiceID} ) {
                    $CheckIfServiceID = 1;
                }
            }
        }

        if ( $QueueIDs[0] && $TypeIDs[0] && $ServiceIDs[0] ) {
            if ( $CheckIfQueueID == 1 && $CheckIfTypeID == 1 && $CheckIfServiceID == 1 ) {
                $SetChecklistID = $ChecklistID;
            }
        }

        if ( !$QueueIDs[0] && $TypeIDs[0] && $ServiceIDs[0] ) {
            if ( $CheckIfTypeID == 1 && $CheckIfServiceID == 1 ) {
                $SetChecklistID = $ChecklistID;
            }
        }

        if ( !$QueueIDs[0] && !$TypeIDs[0] && $ServiceIDs[0] ) {
            if (  $CheckIfServiceID == 1 ) {
                $SetChecklistID = $ChecklistID;
            }
        }
        ...
        if ( $QueueIDs[0] && $TypeIDs[0] && !$ServiceIDs[0] ) {
            if ( $CheckIfQueueID == 1 && $CheckIfTypeID == 1 ) {
                $SetChecklistID = $ChecklistID;
            }
        }

        if ( $QueueIDs[0] && !$TypeIDs[0] && $ServiceIDs[0] ) {
            if ( $CheckIfQueueID == 1 && $CheckIfServiceID == 1 ) {
                $SetChecklistID = $ChecklistID;
            }
        }
    ...
    if ( $SetChecklistID >= 1 ) {
        ...
        my %ChecklistFieldValueListCheck = $ChecklistFieldValueObject->ChecklistFieldValueList(
            ChecklistID => $SetChecklistID,
            TicketID    => $Self->{TicketID},
        );

        if ( !%ChecklistFieldValueListCheck ) {
            ...
            for my $ChecklistFieldListID ( sort { $ChecklistFieldList{$a} <=> $ChecklistFieldList{$b} } keys %ChecklistFieldList ) {
                ...
                my $ChecklistFieldValueID = $ChecklistFieldValueObject->ChecklistFieldValueAdd(
                    ChecklistID => $ChecklistField{ChecklistID},
                    TicketID    => $Self->{TicketID},
                    Task        => $ChecklistField{Task},
                    FieldType   => $ChecklistField{FieldType},
                    Order       => $ChecklistField{Order},
                    UserID      => $Self->{UserID},
                );
        ...
        my $ChecklistValue      = 0;
        my $ChecklistValueOk    = 0;
        my $ChecklistValueClose = 0;
        my $ChecklistValueOpen  = 0;
        if ( %ChecklistFieldValueList ) {

            for my $ChecklistFieldValueID ( sort { $ChecklistFieldValueList{$a} <=> $ChecklistFieldValueList{$b} } keys %ChecklistFieldValueList ) {

                my %ChecklistFieldValue = $ChecklistFieldValueObject->ChecklistFieldValueGet(
                    ChecklistFieldValueID => $ChecklistFieldValueID,
                );

                if ( $ChecklistFieldValue{FieldType} eq "Task" ) {
                    $ChecklistValue ++;
                }

                if ( $ChecklistFieldValue{IfSet} ) {

                    if ( $ChecklistFieldValue{IfSet} == 1 ) {
                        $ChecklistValueOk ++;
                    }
    
                    if ( $ChecklistFieldValue{IfSet} == 2 ) {
                        $ChecklistValueClose ++;
                    }
                }
            }
        ...
        $ChecklistValueOpen = $ChecklistValue - $ChecklistValueOk;
        $ChecklistValueOpen = $ChecklistValueOpen - $ChecklistValueClose;

        $Checklist{ChecklistValueText} = "$ChecklistValueOk = Fertig - $ChecklistValueClose = Nicht beötigt - $ChecklistValueOpen von $ChecklistValue noch offen";

        $Checklist{ChecklistValue} = $ChecklistValue;
        $Checklist{ChecklistValueOk} = $ChecklistValueOk;
        $Checklist{ChecklistValueClose} = $ChecklistValueClose;
        $Checklist{ChecklistValueOpen} = $ChecklistValueOpen;

        if ( $Self->{ChecklistWidget} ) {
            if ( $Self->{ChecklistWidget} eq 'Expanded' ) {
                $Checklist{ChecklistWidget} = $Self->{ChecklistWidget};
            }
            else {
                $Checklist{ChecklistWidget} = 'Collapsed';
            }
        }
        else {
            $Checklist{ChecklistWidget} = 'Collapsed';
        ...
        if ( %ChecklistFieldValueList ) {
            ...
            for my $ChecklistFieldValueID ( sort { $ChecklistFieldValueList{$a} <=> $ChecklistFieldValueList{$b} } keys %ChecklistFieldValueList ) {
                ...
                $ChecklistFieldValue{ChangeBy} = $UserObject->UserName( UserID => $ChecklistFieldValue{ChangeBy} );

                if ( $ChecklistFieldValue{FieldType} eq "Headline" ) {

                    $ChecklistFieldValue{Top} = 'margin-top:20px;';
                    $ChecklistFieldValue{Color} = 'color:#000000;';
                    $ChecklistFieldValue{Bold} = 'font-weight:bold;';
                }

                $LayoutObject->Block(
                    Name => 'OverviewListFieldTaskLabel',
                    Data => { %Param, %ChecklistFieldValue, },
                );
                ...
                if ( $ChecklistFieldValue{FieldType} eq "Task" ) {
                    ...
                    if ( !$ChecklistFieldValue{IfSet}  ) {
                        ...
                        $LayoutObject->Block(
                            Name => 'OverviewListFieldTaskOpen',
                            Data => { %Param, %ChecklistFieldValue, %Checklist, },
                        );
                    }
                    
                    if ( $ChecklistFieldValue{IfSet} ) {

                        if ( $ChecklistFieldValue{IfSet} == 0  ) {
                        ...
                        if ( $ChecklistFieldValue{IfSet} == 1 ) {
                            ...
                            $LayoutObject->Block(
                                Name => 'OverviewListFieldTaskReady',
                                Data => { %Param, %ChecklistFieldValue, },
                            );
                        }
                        ...
                        if ( $ChecklistFieldValue{IfSet} == 2 ) {
                            ...
                            $LayoutObject->Block(
                                Name => 'OverviewListFieldTaskNotRequired',
                                Data => { %Param, %ChecklistFieldValue, },
                            );
                        }
                    }
    ...
    # Checklist management end

    # New process management
    if ( $Ticket{ProcessID} && $Ticket{ProcessID} >= 1 ) {
        ...
        $LayoutObject->Block(
            Name => 'ProcessTicket',
            Data => { %Param, %ProcessData, },
        );

        if ( $ProcessData{Ready} ) {

            $LayoutObject->Block(
                Name => 'ProcessEndReady',
                Data => { %Param, %ProcessData, },
            );
        }
        ...
        if ( $Self->{ProcessError} ) {
            ...
            my %ProcessStepErrorDetail = $TicketProcessStepObject->ProcessStepGet(
                ID => $Self->{ProcessError},
            );

            $LayoutObject->Block(
                Name => 'ProcessError',
                Data => { %Param, %ProcessStepErrorDetail, },
            );
        }

        my @ProcessStepValue = $TicketProcessStepObject->ProcessStepValue(
        ...
        for my $ProcessValue ( 1...$AllValue ) {
            ...
            my %ProcessStep = $TicketProcessStepObject->ProcessStepListValue(
                ProcessID => $Ticket{ProcessID},
                Value     => $ProcessValue,
            );
            ...
            for my $ProcessStepID ( sort { $ProcessStep{$a} <=> $ProcessStep{$b} } keys %ProcessStep ) {
                ...
                %ProcessStepDetail = $TicketProcessStepObject->ProcessStepGet(
                    ID => $ProcessStepID,
                );

                $NextProcessStepID = $TicketProcessStepObject->SearchNextProcessStepWithConditions(
                    ProcessID   => $Ticket{ProcessID},
                ...
                if ( $NextProcessStepID ) {
                    ...
                    %ProcessStepDetailNextCheck = $TicketProcessStepObject->ProcessStepGet(
                ...
                my %ProcessFieldList = $TicketProcessFieldsObject->ProcessFieldList(
                    ProcessID     => $Ticket{ProcessID},
                    ProcessStepID => $ProcessStepID,
                );
                my $ProcessFieldSize = keys %ProcessFieldList;

                my %DynamicProcessFieldList = $TicketDynamicProcessFieldsObject->DynamicProcessFieldList(
                    ProcessID     => $Ticket{ProcessID},
                    ProcessStepID => $ProcessStepID,
                );
                ...
                if ( $ProcessStepDetail{StepArtID} == 2 ) {
                    ...
                    $ProcessStepDetail{ProcessStepArt} = 'process step';
                }
                ...
                if ( $SetDivValue == 1 ) {
                    ...
                    if ( $ProcessStepDetail{StepActive} && $ProcessStepDetail{StepActive} == 1 ) {
                ...
                if ( $ProcessStepDetail{WithoutConditionEnd} && $ProcessStepDetail{WithoutConditionEnd} == 1 ) {
                    ...
                    if ( $ProcessStepDetail{StepActive} == 1 ) {
                        $ProcessStepDetail{ProcessOpacity} = 1;
                    }
                    else {
                        $ProcessStepDetail{ProcessOpacity} = '0.6';
                    }
                    ...
                    if ( $ProcessStepDetail{ToIDFromOne} ) {
                        ...
                        %ProcessStepDetailNext = $TicketProcessStepObject->ProcessStepGet(
                            ID => $ProcessStepDetail{ToIDFromOne},
                        );
                        if ( $ProcessStepDetailNext{StepArtID} == 1 ) {
                            $ProcessStepDetailNext{ProcessStepArt} = 'process step';
                        }
                        else {
                    ...
                    if ( $ProcessStepDetail{ToIDFromTwo} ) {
                        ...
                        %ProcessStepDetailNextToTwo = $TicketProcessStepObject->ProcessStepGet(
                            ID => $ProcessStepDetail{ToIDFromTwo},
                        );
                        if ( $ProcessStepDetailNextToTwo{StepArtID} == 1 ) {
                            $ProcessStepDetailNextToTwo{ProcessStepArt} = 'process step';
                        }
                        else {
