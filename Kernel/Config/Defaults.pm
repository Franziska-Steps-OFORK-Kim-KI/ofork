...
# --
# $Id: Defaults.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
# Default configuration for OFORK. All changes to this file will be lost after an
...
# Perl 5.16.0 is the required minimum version to use OFORK.
use 5.016_000;
...
loads the default values of settings that are required to run OFORK even
...
sub LoadDefaults {
    ...
    $Self->{AdminEmail} = 'support@<OFORK_CONFIG_FQDN>';
    ...
    $Self->{DatabaseDSN} = "DBI:mysql:database=<OFORK_CONFIG_Database>;host=<OFORK_CONFIG_DatabaseHost>;";

    # The database DSN for PostgreSQL ==> more: "perldoc DBD::Pg"
#    $Self->{DatabaseDSN} = "DBI:Pg:dbname=<OFORK_CONFIG_Database>;host=<OFORK_CONFIG_DatabaseHost>;";
...
#    };
    ...
    # (URL JavaScript path.)
    $Self->{'Frontend::JavaScriptPath'} = '<OFORK_CONFIG_Frontend::WebPath>js/';

    # Frontend::CSSPath
    # (URL CSS path.)
    $Self->{'Frontend::CSSPath'} = '<OFORK_CONFIG_Frontend::WebPath>css/';

    # Frontend::ImagePath
    # (URL image path of icons for navigation.)
    $Self->{'Frontend::ImagePath'} = '<OFORK_CONFIG_Frontend::WebPath>skins/Agent/default/img/';
...
#    $Self->{'LogModule::SysLog::Charset'} = 'utf-8';
    ...
    $Self->{'LogModule::LogFile'} = '/tmp/ofork.log';
...
#    $Self->{'SendmailNotificationEnvelopeFrom'} = '';
    ...
    # (enable what you need, auth against ofork db,        #
    # against LDAP directory, against HTTP basic auth     #
    # or against Radius server)                           #
    # --------------------------------------------------- #
    # This is the auth. module against the ofork db
...
#    $Self->{'AuthModule::LDAP::UID'} = 'uid';
    ...
    # (e. g. user needs to be in a group xyz to use ofork)
#    $Self->{'AuthModule::LDAP::GroupDN'} = 'cn=oforkallow,ou=posixGroups,dc=example,dc=com';
...
#    $Self->{'AuthModule::LDAP::UserLowerCase'} = 0;
    ...
    # In case you need to use OFORK in iso-charset, you can define this
...
#    ];
    ...
    # groups to ofork groups, define the following.)
...
#    };
    ...
    # (If "LDAP" was selected for AuthModule and you want to sync LDAP
    # groups to ofork roles, define the following.)
...
#    };
    ...
    # (If "LDAP" was selected for AuthModule and you want to sync LDAP
    # attributes to ofork groups, define the following.)
...
#    };
    ...
    # (If "LDAP" was selected for AuthModule and you want to sync LDAP
    # attributes to ofork roles, define the following.)
...
#    $Self->{SessionModule} = 'Kernel::System::AuthSession::FS';
    ...
    # SessionName
    # (Name of the session key. E. g. Session, SessionID, OFORK)
    $Self->{SessionName} = 'OFORKAgentInterface';
    ...
    # $Self->{SessionModule}='Kernel::System::AuthSession::FS)
    $Self->{SessionDir} = '<OFORK_CONFIG_Home>/var/sessions';
    ...
    # --------------------------------------------------- #
    # TimeZone
    # (set the OFORK time zone, default is UTC)
#    $Self->{'OFORKTimeZone'} = 'UTC';
...
#    $Self->{'MIME-Viewer'}->{'text/xml'} = $Self->{Home}.'/scripts/tools/xml2html.pl';
    ...
    # tmp dir
    $Self->{TempDir} = '<OFORK_CONFIG_Home>/var/tmp';

    # article dir
    $Self->{'Ticket::Article::Backend::MIMEBase::ArticleDataDir'} = '<OFORK_CONFIG_Home>/var/article';

    # html template dirs
    $Self->{TemplateDir}       = '<OFORK_CONFIG_Home>/Kernel/Output';
    $Self->{CustomTemplateDir} = '<OFORK_CONFIG_Home>/Custom/Kernel/Output';
    ...
    $Self->{'Package::RepositoryRoot'} = [
        'https://ftp.o-fork.de/pub/ofork/misc/packages/repository.xml',
    ];

    # Package::RepositoryList
    # (repository list)
#    $Self->{'Package::RepositoryList'} = {
#        'ftp://ftp.example.com/pub/ofork/misc/packages/' => '[Example] ftp://ftp.example.com/',
#    };
...
#    $Self->{'Package::Proxy'} = 'http://proxy.sn.no:8001/';
    ...
    $Self->{'PGP::Options'} = '--homedir /opt/ofork/.gnupg/ --batch --no-tty --yes';
...
#    $Self->{'SMIME::PrivatePath'} = '/etc/ssl/private';
    ...
    $Self->{PreferencesGroups}->{Theme} = {
        ...
        'Key'     => '',
        'Label'   => 'Theme',
        'Desc'    => 'Select your preferred theme for OFORK.',
    ...
    # notification sender
    $Self->{NotificationSenderName}  = 'OFORK Notifications';
    $Self->{NotificationSenderEmail} = 'ofork@<OFORK_CONFIG_FQDN>';

    # notification email for new password
    $Self->{NotificationSubjectLostPassword} = 'New OFORK password';
    $Self->{NotificationBodyLostPassword}    = 'Hi <OFORK_USERFIRSTNAME>,


Here\'s your new OFORK password.

New password: <OFORK_NEWPW>

You can log in via the following URL:

<OFORK_CONFIG_HttpType>://<OFORK_CONFIG_FQDN>/<OFORK_CONFIG_ScriptAlias>index.pl
            ';
    ...
    # SessionName
    # (Name of the session key. E. g. Session, SessionID, OFORK)
    $Self->{CustomerPanelSessionName} = 'OFORKCustomerInterface';

    # CustomerPanelUserID
    # (The customer panel db-uid.) [default: 1]
    $Self->{CustomerPanelUserID} = 1;

    # CustomerGroupSupport (0 = compat. to OFORK 1.1 or lower)
    # (if this is 1, the you need to set the group <-> customer user
    # relations! http://host/ofork/index.pl?Action=AdminCustomerUserGroup
...
#    $Self->{'CustomerPanel::InfoFile'} = 'CustomerAccept';
    ...
    # --------------------------------------------------- #
    $Self->{CustomerPanelSubjectLostPassword} = 'New OFORK password';
    $Self->{CustomerPanelBodyLostPassword}    = 'Hi <OFORK_USERFIRSTNAME>,


New password: <OFORK_NEWPW>

<OFORK_CONFIG_HttpType>://<OFORK_CONFIG_FQDN>/<OFORK_CONFIG_ScriptAlias>customer.pl
            ';

    # --------------------------------------------------- #
    # notification email about new account                #
    # --------------------------------------------------- #
    $Self->{CustomerPanelSubjectNewAccount} = 'New OFORK Account!';
    $Self->{CustomerPanelBodyNewAccount}    = 'Hi <OFORK_USERFIRSTNAME>,

You or someone impersonating you has created a new OFORK account for
you.

Full name: <OFORK_USERFIRSTNAME> <OFORK_USERLASTNAME>
User name: <OFORK_USERLOGIN>
Password : <OFORK_USERPASSWORD>
...
<OFORK_CONFIG_HttpType>://<OFORK_CONFIG_FQDN>/<OFORK_CONFIG_ScriptAlias>customer.pl
    ...
    # --------------------------------------------------- #
    # This is the auth. module for the ofork db
...
#    $Self->{'Customer::AuthModule::LDAP::UID'} = 'uid';
    ...
    # Check if the user is allowed to auth in a posixGroup
    # (e. g. user needs to be in a group xyz to use ofork)
#    $Self->{'Customer::AuthModule::LDAP::GroupDN'} = 'cn=oforkallow,ou=posixGroups,dc=example,dc=com';
...
#    $Self->{'Customer::AuthModule::HTTPBasicAuth::ReplaceRegExp'} = '^(.+?)@.+?$';
    # If you use this module, you should use as fallback the following
    # config settings if user isn't login through apache ($ENV{REMOTE_USER})
#    $Self->{CustomerPanelLoginURL} = 'http://host.example.com/not-authorised-for-ofork.html';
#    $Self->{CustomerPanelLogoutURL} = 'http://host.example.com/thanks-for-using-ofork.html';
...
#            [ 'UserEmail',        Translatable('Email'),               'email',          1, 1, 'var', '[% Env("CGIHandle") %]?Action=AgentTicketCompose;ResponseID=1;TicketID=[% Data.TicketID | uri %];ArticleID=[% Data.ArticleID | uri %]', 0, '', 'AsPopup OFORKPopup_TicketAction' ],
...
sub new {
    ...
    if ( -e !"$Self->{Home}/RELEASE" ) {
        print STDERR
            "ERROR: $Self->{Home}/RELEASE does not exist! This file is needed by central system parts of OFORK, the system will not work without this file.\n";
        die;
    }
    ...
    if ( open( my $Product, '<', "$Self->{Home}/RELEASE" ) ) {
        ...
        print STDERR
            "ERROR: Can't read $Self->{Home}/RELEASE: $! This file is needed by central system parts of OFORK, the system will not work without this file.\n";
        die;
    }
    ...
    if ( !$Param{Level} ) {
        ...
        for my $Key ( sort keys %{$Self} ) {
            ...
            if ( defined $Self->{$Key} ) {
                $Self->{$Key} =~ s/\<OFORK_CONFIG_(.+?)\>/$Self->{$1}/g;
            }
            else {
