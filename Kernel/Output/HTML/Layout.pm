...
# --
# $Id: Layout.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Login {
    ...
    if ( $ConfigObject->Get('SessionUseCookie') ) {
        ...
        $Self->{SetCookies}->{OFORKBrowserHasCookie} = $Kernel::OM->Get('Kernel::System::Web::Request')->SetCookie(
            Key      => 'OFORKBrowserHasCookie',
            Value    => 1,
            Expires  => '+1y',
...
sub Attachment {
    ...
    if ( $Param{Sandbox} && !$Kernel::OM->Get('Kernel::Config')->Get('DisableContentSecurityPolicy') ) {
        ...
        #   been deprecated and will be removed in future OFORK versions in favor of a separate header (see below).
...
sub TransformDateSelection {
    ...
    # from user time zone to OFORK time zone
...
sub CustomerNavigationBar {
    ...
    for my $RoomID ( keys %RoomList ) {
         $CheckBookingRoom ++;
    }

    my $CheckIfFAQ = $ConfigObject->Get('FAQ::ApprovalTicketType');
    my $CustomerFAQArticle = '';
    if ( $CheckIfFAQ ) {
        my $FAQObject = $Kernel::OM->Get('Kernel::System::FAQ');
        my %CustomerFAQArticleList = $FAQObject->CheckIfCustomerArticle(
            UserID => 1,
        );
        if ( %CustomerFAQArticleList ) {
            $CustomerFAQArticle = 1;
        }
    }
    ...
    my $CheckSelfServiceCategories = '';
    my %SelfServiceCategoriesList = $SelfServiceCategoriesObject->SelfServiceCategoriesList(
        Valid  => 1,
        UserID => 1,
    );
    for my $CategorieID ( keys %SelfServiceCategoriesList ) {
         $CheckSelfServiceCategories ++;
    }
    ...
    for my $Module ( sort keys %{$NavigationConfig} ) {
        ...
        next MODULE if !$CheckRequests && $Module eq "CustomerRequest";

        next MODULE if !$CustomerFAQArticle && $Module eq "CustomerFAQExplorer";

        next MODULE if !$CheckBookingRoom && $Module eq "CustomerBookingOverview";

        next MODULE if !$CheckBookingRoom && $Module eq "CustomerRoomBooking";

        next MODULE if !$CustomerProcess && $Module eq "CustomerTicketProcess";

        next MODULE if !$CheckSelfServiceCategories && $Module eq "SelfService";
    ...
    for my $Item ( sort keys %NavBarModule ) {
        ...
        if ( $NavBarModule{$Item}->{Name} eq 'Tickets' ) {
            $Self->Block(
