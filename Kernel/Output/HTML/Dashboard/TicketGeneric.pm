...
# --
# $Id: TicketGeneric.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# ---
...
sub Run {
    ...
    # show only owner if we have the filter
    if ( $TicketSearchSummary{Owner} ) {
        $LayoutObject->Block(
    ...
    for my $TicketID ( @{$TicketIDs} ) {
        ...
        for my $Column (@Columns) {
            ...
            if ( $Column !~ m{\A DynamicField_}xms ) {
                ...
                elsif ( $Column eq 'Process' ) {
                    if ( !$Ticket{ProcessID} || $Ticket{ProcessID} < 1 ) {
                        $DataValue = '';
                    }
                    else {
                        $BlockType = 'Translatable';
                        $DataValue = 'Process ticket';
                    }
                }
...
sub _SearchParamsGet {
    ...
    if ( defined $TicketSearch{OwnerIDs} ) {
        delete $TicketSearchSummary{Owner};
    }
