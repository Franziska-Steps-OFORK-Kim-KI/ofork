...
# --
# $Id: Small.pm,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
sub Run {
    ...
    # Get extended ticket data if columns are used, save performance.
    ...
    for my $ArticleRef (@ArticleBox) {
        ...
        for my $TicketColumn (@Col) {
            ...
            if ( $TicketColumn !~ m{\A DynamicField_}xms ) {
                ...
                elsif ( $TicketColumn eq 'Process' ) {
                    if ( !$Article{ProcessID} || $Article{ProcessID} < 1 ) {
                        $DataValue = '';
                    }
                    else {
                        $BlockType = 'Translatable';
