...
PREPARE FKStatement FROM @FKSQLStatement;
EXECUTE FKStatement;
SET @FKExists := (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_schema = DATABASE() AND table_name = 'article_data_ofork_chat' AND constraint_name = 'FK_article_data_ofork_chat_article_id_id');
SET @FKSQLStatement := IF( @FKExists = 0, 'ALTER TABLE article_data_ofork_chat ADD CONSTRAINT FK_article_data_ofork_chat_article_id_id FOREIGN KEY (article_id) REFERENCES article (id)', 'SELECT ''INFO: Foreign key constraint FK_article_data_ofork_chat_article_id_id does already exist, skipping.''' );
PREPARE FKStatement FROM @FKSQLStatement;
EXECUTE FKStatement;
