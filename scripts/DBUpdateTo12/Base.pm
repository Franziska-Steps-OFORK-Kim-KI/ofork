...
package scripts::DBUpdateTo12::Base;
...
scripts::DBUpdateTo12::Base - Base class for migrations.
...
Refreshes the configuration to make sure that a ZZZAAuto.pm is present after the upgrade.
    ...
    $DBUpdateTo12Object->RebuildConfig(
...
Clean up the cache.
    ...
    $DBUpdateTo12Object->CacheCleanup();
...
Parse and execute an XML array.
    ...
    $DBUpdateTo12Object->ExecuteXMLDBArray(
        XMLArray          => \@XMLArray,
...
sub ExecuteXMLDBArray {
    ...
    for my $XMLString ( @{ $Param{XMLArray} } ) {
        ...
        # alter table (without renaming the table!)
            ...
            if ( $XMLString =~ m{ <ColumnChange \s+ NameOld="([^"]+)" }xms ) {
...
Parse and execute an XML string.
    ...
    $DBUpdateTo12Object->ExecuteXMLDBString(
        XMLString => '
...
Checks if the given table exists in the database.
    ...
    my $Result = $DBUpdateTo12Object->TableExists(
        Table => 'ticket',
    );
...
Checks if the given column exists in the given table.
    ...
    my $Result = $DBUpdateTo12Object->ColumnExists(
        Table  => 'ticket',
        Column =>  'id',
    );
...
Checks if the given index exists in the given table.
    ...
    my $Result = $DBUpdateTo12Object->IndexExists(
        Table => 'ticket',
        Index =>  'id',
    );
...
Clean up the cache.
    ...
    $DBUpdateTo12Object->GetTaskConfig( Module => "TaskModuleName");
