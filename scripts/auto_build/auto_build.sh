...
if ! test $PATH_TO_CVS_SRC || ! test $VERSION || ! test $RELEASE; then
    ...
    echo ""
    echo "  Try: auto_build.sh /home/ernie/src/ofork 7.0.1 01"
    echo ""
    exit 1;
else
    # --
    ...
    if ! test -e $PATH_TO_CVS_SRC/RELEASE; then
        echo "Error: $PATH_TO_CVS_SRC is not OFORK CVS directory!"
        exit 1;
    fi
fi
...
# mk ARCHIVE
bin/ofork.CheckSum.pl -a create
...
CreateRPM "SuSE 12"   "suse12-ofork.spec"   "suse/12/"
CreateRPM "SuSE 13"   "suse13-ofork.spec"   "suse/13/"
CreateRPM "Fedora 25" "fedora25-ofork.spec" "fedora/25/"
CreateRPM "Fedora 26" "fedora26-ofork.spec" "fedora/26/"
CreateRPM "RHEL 7"    "rhel7-ofork.spec"    "rhel/7"
...
if which md5sum >> /dev/null; then
    ...
    find . -name "*$PACKAGE*" | xargs md5sum | sed -e "s/^/| /" -e "s/\.\//| https:\/\/ftp.o-fork.de\/\//" -e "s/$/ |/"
else
