...
Restore an OFORK system from backup.

Usage:
 restore.pl -b /data_backup/<TIME>/ -d /opt/ofork/

Options:
 -b                     - Directory of the backup files.
 -d                     - Target OFORK home directory.
...
if ( $DB =~ m/mysql/i ) {
    ...
    $Kernel::OM->Get('Kernel::System::DB')->Prepare(
        SQL =>
            "SELECT table_name FROM information_schema.tables WHERE table_catalog = 'ofork' AND table_schema = 'public'",
    );
    my $Check = 0;
    while ( my @RowTmp = $Kernel::OM->Get('Kernel::System::DB')->FetchrowArray() ) {
        $Check++;
