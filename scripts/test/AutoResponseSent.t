...
for my $Test (@Tests) {
    ...
    # check auto response suppression with X-OFORK-Loop
...

    {
        ...
        $Self->True(
            ...
            "Test $Count : Emails fetched from backend - AutoResponse $Test->{AutoResponseType} suppressed by X-OFORK-Loop",
        );
    }
    ...
    # check auto response re-enabling with X-OFORK-Loop
    $ArticleIDOne = $ArticleBackendObject->ArticleCreate(
        TicketID             => $TicketIDOne,
        IsVisibleForCustomer => 1,
        SenderType           => 'customer',
...

    {
        ...
        $Self->True(
            $LastArticleMailSent && ( scalar @{$Emails} ) == 1,
            "Test $Count : Emails fetched from backend - AutoResponse $Test->{AutoResponseType} re-enabled by X-OFORK-Loop",
        );
    }
