...
$Self->Is(
    $Values->{TimeZone},
    $DateTimeObject->OFORKTimeZoneGet(),
...
my $OriginalOFORKTimeZone = $DateTimeObject->OFORKTimeZoneGet();
...
$Self->IsDeeply(
    ...
    'Date and time after call to ToOFORKTimeZone must match expected values.'
);
...
for my $TestConfig (@TestConfigs) {
    ...
    for my $ValueName (qw ( Year Month Day Hour Minute Second TimeZone )) {
        ...
        if ( !$ExpectedValue && $ValueName eq 'TimeZone' ) {
            $ExpectedValue = $DateTimeObject->OFORKTimeZoneGet();
        }
