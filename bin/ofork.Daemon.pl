...
# --
# $Id: ofork.Daemon.pl,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
print STDOUT "\nManage the OFORK daemon process.\n\n";

local $Kernel::OM = Kernel::System::ObjectManager->new(
    'Kernel::System::Log' => {
        LogPrefix => 'OFORK-ofork.Daemon.pl',
    },
);
...
if ( $> == 0 ) {    # $EFFECTIVE_USER_ID
    print STDERR
        "Error: You cannot run ofork.Daemon.pl as root. Please run it as the 'ofork' user or with the help of su:\n";
    print STDERR "  su -c \"bin/ofork.Daemon.pl ...\" -s /bin/bash ofork\n";
    exit 1;
}
...
sub PrintUsage {
    my $UsageText = "Usage:\n";
    $UsageText .= " ofork.Daemon.pl action [--debug] [--force]\n";
    $UsageText .= "\nOptions:\n";
    ...
    while ($DaemonChecker) {
        ...
        for my $Module ( sort keys %DaemonModules ) {
            ...
            if ( !$ChildPID ) {
                ...
                local $Kernel::OM = Kernel::System::ObjectManager->new(
                    'Kernel::System::Log' => {
                        LogPrefix => "OFORK-ofork.Daemon.pl - Daemon $Module",
                    },
                );
...
sub _LogFilesSet {
    ...
    # get log rotation type and backup old logs if logs should be rotated by OFORK
    my $RotationType = lc $ConfigObject->Get('Daemon::Log::RotationType') || 'ofork';
    if ( $RotationType eq 'ofork' ) {
    ...
    return 1 if $RotationType ne 'ofork';

    # remove not needed log files if OFORK rotation is enabled
...
sub _LogFilesCleanup {
    ...
    # skip cleanup if OFORK log rotation is not enabled
    my $RotationType = lc $Kernel::OM->Get('Kernel::Config')->Get('Daemon::Log::RotationType') || 'ofork';
    return 1 if $RotationType ne 'ofork';
