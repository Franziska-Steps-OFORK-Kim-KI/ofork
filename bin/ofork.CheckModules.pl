...
# --
# $Id: ofork.CheckModules.pl,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
# check needed params
if ($Help) {
    print "\nReturn all required and optional packages of OFORK.\n\n";
    print "Usage:\n";
    print " ofork.CheckModules.pl [-list|all]\n\n";
    print "Options:\n";
    printf " %-22s - %s", '[-list]', 'Return an install command with all required packages.' . "\n";
    printf " %-22s - %s", '[-all]',  'Return all required, optional and bundled packages of OFORK.' . "\n\n";
    exit 1;
}
...
my @NeededModules = (
    ...
    },
    {
        ...
        VersionsRecommended => [
            {
                Version => '2.066',
                Comment => 'This version fixes email sending.',
            },
        ],
        InstTypes => {
    ...
    },
    {
        Module    => 'Sys::Syslog',
        Required  => 0,
        Comment   => 'Perl interface to the UNIX syslog calls.',
        InstTypes => {
            aptget => 'libsys-syslog-perl',
            emerge => 'dev-perl/Sys-Syslog',
            zypper => 'perl-Sys-Syslog',
            ports  => 'www/p5-Sys-Syslog',
        },
    },
    {
        Module    => 'Template',
        Required  => 1,
        Comment   => 'Template::Toolkit, the rendering engine of OFORK.',
        InstTypes => {
