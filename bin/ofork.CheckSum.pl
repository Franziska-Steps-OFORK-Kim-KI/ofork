#!/usr/bin/perl
# --
# bin/ofork.CheckSum.pl
# Modified version of the work:
# Copyright (C) 2010-2025 OFORK, https://o-fork.de
# based on the original work of:
# Copyright (C) 2001-2018 OTRS AG, http://otrs.com/
# --
# $Id: ofork.CheckSum.pl,v 1.1.1.1 2018/07/16 14:49:06 ud Exp $
# --
...
Create or compare OFORK file checksum information.

Usage:
 ofork.CheckSum.pl -a create|compare [-b /path/to/ARCHIVE] [-d /path/to/framework]

Options:
 -a                     - Specify the action (create|compare).
 [-b]                   - Specify the path to archive.
 [-d]                   - Specify the path to OFORK framework.
...
sub ProcessDirectory {
    ...
    for my $File (@List) {
        ...
        # always stay in OFORK directory
        ...
        if ( $Action eq 'create' ) {
            ...
            elsif ( $Compare{$File} ne $Digest && !-e "$File.save" ) {
                print "Notice: Dif $File\n";
            }
            elsif ( -e "$File.save" )
            {    ## report .save files as modified by the OFORK Package Manager
